"""Skeletal payload rules shared by the GLB stage and the temporary legacy producer.

Derivation, basis, root, masks and section serialization have one implementation.
The source adapter supplies decoded records; this module performs no install or file reads.
"""
import struct

from elysium_pipeline.asset_names import rig_bone_name
from elysium_pipeline.formats import bsp
from elysium_pipeline.skeletal_stage import source_api as S
...
from elysium_pipeline.formats.mesh_geometry import unreal_surface_normals as _surface_normals
...
def _bone_mask(d, bones, clip, bone_map, emitted):
    ...
    pose on the second.
    """
    weights = S.bone_weights(d, clip, bones)
...
def _owned_bones(d, bones, clip):
    ...
    weights = S.bone_weights(d, clip, bones)
    return {b.index for b in bones if weights[b.index] != 0.0}
...
def _authored_channels(d, bones, clip):
    ...
    """
    return S.authored_channels(d, bones, clip)
