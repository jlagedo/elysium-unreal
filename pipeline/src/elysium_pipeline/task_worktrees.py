...
class TaskWorktreeRecord:
    ...
    base_commit: str
    created_at: str
    #: The engine copy this task builds against. UnrealBuildTool's single-instance mutex is
    #: keyed on its own assembly path, so two checkouts sharing one engine root serialize
    #: their builds against each other. Absent in records written before per-task engines.
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        value["ue_root"] = None if self.ue_root is None else str(self.ue_root)
        return value
    ...
    def load(cls, path: Path) -> "TaskWorktreeRecord":
        ...
        try:
            record = cls(
                ...
                ue_root=(
                    Path(value["ue_root"]).resolve()
                    if value.get("ue_root")
                    else None
                ),
            )
...
def create_task_worktree(
    *,
    ...
    engine = validate_engine_root(ue_root)
    ...
    try:
        ...
        (worktree / ".elysium.local.env").write_text(
            "\n".join(
                (
                    ...
                    f"ELYSIUM_UE_ROOT={_quoted_environment_value(engine)}",
        ...
        if max_parallel_actions is not None:
            write_build_parallelism(worktree, max_parallel_actions)
...
def task_worktree_status(
    ...
    status: dict[str, Any] = {
        ...
        "ue_root": None if record.ue_root is None else str(record.ue_root),
        "base_commit": record.base_commit,
        "head": None,
        "missing": not record.worktree.is_dir(),
        "dirty": [],
        "unlanded_commits": [],
    ...
    if status["missing"]:
        return status
    # A checkout Git no longer owns is state to report, not a reason to abandon the listing:
    # status is the surface that has to keep working once a task worktree has broken, and one
    # unreadable record must not hide the healthy ones beside it.
    try:
        head = assert_task_worktree_runnable(record, runner)
        status["head"] = head
        status["dirty"] = list(_dirty_rows(runner, record.worktree))
        status["unlanded_commits"] = list(_unlanded_commits(record, runner, head))
    except TaskWorktreeError as exc:
...
def close_task_worktree(
    ...
    with WorkspaceLease(
        ...
        orphaned = not record.worktree.is_dir()
        if not orphaned:
            try:
                assert_task_worktree_runnable(record, runner)
            except TaskWorktreeError:
                orphaned = True
        if orphaned:
            # The checkout is gone or Git no longer owns it, so it holds no commit that
            # landing could protect. Closing prunes the record and the generated work root
            # and leaves any files behind, because their contents are no longer known here.
            _git(runner, record.source_repo, "worktree", "prune")
        else:
            head = assert_task_worktree_runnable(record, runner)
