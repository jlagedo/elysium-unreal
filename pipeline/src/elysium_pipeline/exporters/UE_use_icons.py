...
`docs/vtmb/entity_io.md`); `use_icon N` selects entry `N-1`, `use_icon 0` = no icon. This
...
Output is map-independent, so it lands in the global `$ELYSIUM_EXPORT_ROOT/hud/` folder (next to the
per-map `$ELYSIUM_EXPORT_ROOT/<map>/` dirs), read 1:1 by the runtime -- no coordinate space is involved
(2D HUD art), which is why this is a `UE_` writer despite touching no geometry.

Produces (under $ELYSIUM_EXPORT_ROOT/hud/):
...
OUT = os.path.join(os.fspath(export_root()), "hud")

# The 72-entry use_icon enum, verbatim from docs/vtmb/entity_io.md (client.dll:0x2700b4).
