"""Gapless source-byte accountability for material members.

A VMT is text, so every byte belongs to exactly one lexical region: a token the decode read, a
comment it captured verbatim, or the insignificant whitespace the KeyValues grammar discards.
Claiming all three is what lets the unit publish no opaque copy of the source.
"""
...
#: A VMT reaches neither: text has no alignment fill and no declared-zero field.
ALLOWED_STATES = {
    "mapped",
    "derived",
    "omitted-proven",
...
class ByteLedger:
    ...
    def claim(self, offset: int, length: int, state: str, owner: str) -> None:
        if state not in ALLOWED_STATES:
            raise MaterialByteCoverageError(f"{self.path}: invalid state {state!r}")
