"""Complete character-facing projection of a legacy VPhysics PHY companion."""
...
class CharacterPhysicsError(ValueError):
...
def _leaf_ledges(
    ...
    if node in visited:
        raise CharacterPhysicsError(f"ledge tree revisits node at {node}")
    if node < 0 or node + LEDGE_NODE_SIZE > len(data):
        raise CharacterPhysicsError(f"ledge node at {node} runs outside the PHY")
    visited.add(node)
    ...
    for triangle in source_triangles:
        for source in triangle:
            ...
            source_points.append(
                {
                    "index": source,
                    "ivp": (float(x), float(y), float(z), float(w)),
                    "position": position,
                }
            )
...
def decode(data: bytes) -> dict[str, Any]:
    ...
    if len(data) < HEADER.size:
        raise CharacterPhysicsError(f"PHY is only {len(data)} bytes")
    ...
    for index in range(solid_count):
        if position + 4 > len(data):
            raise CharacterPhysicsError(f"solid {index} has no size field")
        ...
        if magic not in (b"IVPS", b"\0\0\0\0"):
            raise CharacterPhysicsError(f"solid {index} has magic {magic!r}")
    ...
    if len(solid_blocks) != solid_count:
        raise CharacterPhysicsError(
            f"PHY declares {solid_count} binary solids but {len(solid_blocks)} solid blocks"
        )
    ...
    return {
        ...
        "keyValues": [
            {
                "type": block["type"],
                "values": _typed_values(block["type"], block["values"]),
                "pairs": block["pairs"],
            }
            for block in blocks
        ],
