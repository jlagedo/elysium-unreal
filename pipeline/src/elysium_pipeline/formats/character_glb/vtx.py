"""Complete-LOD MDL/VTX geometry decoder for the Character GLB Exporter."""
...
class CharacterTopologyError(ValueError):
...
def _normal(
    ...
    if vertex_list == 1:
        ...
        raise CharacterTopologyError(f"unknown embedded vertex list {vertex_list}")
...
def decode_header(data: bytes) -> dict[str, Any]:
    if len(data) < 36:
        raise CharacterTopologyError(f"VTX is only {len(data)} bytes")
    return {
        "version": _i32(data, 0),
...
def decode_lods(
    ...
    *,
    ...
    if mdl_data[:4] != b"IDST":
        raise CharacterTopologyError(f"bad MDL ident {mdl_data[:4]!r}")
    if len(vtx_data) < 36:
        raise CharacterTopologyError(f"{variant} VTX is only {len(vtx_data)} bytes")
            ...
            for lod_index in range(lod_count):
                ...
                for mesh_index in range(mesh_count):
                    ...
                    for stripgroup_index in range(stripgroup_count):
                        ...
                        index_records = [
                            _u16(vtx_data, index_table + index * 2)
                            for index in range(table_index_count)
                        ]
                        vertex_records = []
                        ...
                        for strip_index in range(strip_count):
                            ...
                            strip_indices = [
                                _u16(vtx_data, index_table + (first_index + item) * 2)
