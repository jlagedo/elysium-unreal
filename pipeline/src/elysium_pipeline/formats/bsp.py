...
def leaf_areas(data):
    ...
    lists, so the pass sees exactly one area's leaves (`../docs/vtmb/sky-ambience.md` -> "The 3D
...
def read_worldlights(data, raw_values=False):
    ...
    **The engine's own load-time fixups are applied**, because `Mod_LoadWorldlights` is not a
    ...
    (`../docs/vtmb/sky-ambience.md` -> "What the loader changes on the way in", RE-A3):
