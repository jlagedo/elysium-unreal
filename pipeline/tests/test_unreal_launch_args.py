...
def test_an_already_present_flag_is_not_duplicated() -> None:
...
def test_a_non_editor_executable_is_untouched() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        config = _config(temporary)
        runner = RecordingRunner()
...
def test_auxiliary_policy_content_is_its_own_pair_of_launches() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        config = _config(temporary)
        runner = RecordingRunner()
