...
def test_selected_malkavian_chain_becomes_one_native_recipe():
    bones = _bones(
    ...
    assert len(chains) == 1
    chain = chains[0]
    assert (chain.first_bone, chain.chain_end) == ("Bone05", "Bone09")
    assert chain.gravity_scale == pytest.approx(0.9, abs=1e-7)
    assert chain.damping == pytest.approx(0.9, abs=1e-7)
    assert chain.angular_spring == pytest.approx(4.0 * math.pow(10.0, -0.3), abs=1e-6)
    assert chain.cone_angle_degrees == pytest.approx(60.0, abs=1e-7)


def test_selected_jeanette_routes_are_exact_and_other_models_are_empty():
    bones = _bones(
    ...
    assert [(item.first_bone, item.chain_end) for item in selected] == [
        ("Bone01", "Bone07"), ("Bone09", "Bone13")]
    assert motion.anim_dynamics_poc_chains(THERESE, blob, bones) == []


def test_malformed_chain_table_and_non_finite_values_fail_loudly():
...
def test_selected_chain_must_exist_and_be_head_parented():
    bones = _bones(["Bip01 Spine1", "Bone05", "Bone06"], [-1, 0, 1])
    blob = _model([(1, -1, 0.0, 0.9, 0.9, 0.3, 60.0)])
    with pytest.raises(ValueError, match="not 'Bip01 Head'"):
        motion.anim_dynamics_poc_chains(MALKAVIAN, blob, bones)


def test_malkavian_anonymous_breasts_are_bodies_and_not_hair():
    bones = _bones(
        ["Bip01 Head", "Bone05", "Bone06", "Bone07", "Bone08", "Bone09",
    ...
    assert len(chains) == 1
    assert [body.bound_bone for body in bodies] == ["Bone01", "Bone03"]
    assert bodies[0].gravity_scale == pytest.approx(0.0, abs=1e-7)
    assert bodies[0].damping == pytest.approx(0.95, abs=1e-7)
    assert bodies[0].angular_spring == pytest.approx(4.0 * math.pow(10.0, -0.3), abs=1e-6)
    assert bodies[0].cone_angle_degrees == pytest.approx(30.0, abs=1e-7)


def test_jeanette_token_breasts_are_bodies_beside_hair():
    bones = _bones(
        ["Bip01 Head", "Bone01", "Bone03", "Bone05", "Bone06", "Bone07",
         "Bone09", "Bone10", "Bone11", "Bone12", "Bone13",
    ...
    assert [(item.first_bone, item.chain_end) for item in chains] == [
        ("Bone01", "Bone07"), ("Bone09", "Bone13")]
    assert [body.bound_bone for body in bodies] == ["right breast", "left breast"]
    assert bodies[0].cone_angle_degrees == pytest.approx(90.0, abs=1e-7)
    assert bodies[0].angular_spring == pytest.approx(4.0 * math.pow(10.0, -2.0), abs=1e-6)


def test_therese_has_breast_bodies_and_no_hair_recipe():
    bones = _bones(
...
def test_token_names_and_underscore_forms_are_admitted():
    bones = _bones(
    ...
    assert [body.bound_bone for body in bodies] == ["BoobLeft03", "left_breast"]
    assert bodies[1].gravity_scale == pytest.approx(3.0, abs=1e-7)
    assert bodies[1].cone_angle_degrees == pytest.approx(15.0, abs=1e-7)


def test_authored_cone_above_90_is_capped_for_breasts():
...
def test_damping_below_animdynamics_floor_is_clamped():
    bones = _bones(["Bip01 Spine1", "left breast"], [-1, 0])
    blob = _model([(1, -1, 0.0, 0.0, 0.15, 0.3, 30.0)])
    bodies = motion.anim_dynamics_breast_bodies(LILY, blob, bones)
    assert bodies[0].damping == pytest.approx(0.7, abs=1e-7)


def test_head_parented_one_bone_curl_is_not_a_breast():
    ...
    assert [body.bound_bone for body in bodies] == ["left breast"]


def test_three_bone_spine_chain_is_not_a_breast():
    bones = _bones(
        ["Bip01 Spine", "Bone01", "Bone02", "Bone03"],
        [-1, 0, 1, 2])
    blob = _model([(1, -1, 0.0, 3.0, 0.9, 0.3, 30.0)])
    assert motion.anim_dynamics_breast_bodies(TEMPLE, blob, bones) == []


def test_tzimisce_rib_singletons_are_rejected():
    bones = _bones(
    ...
    assert motion.anim_dynamics_breast_bodies(TZIMISCE, blob, bones) == []


def test_unclassified_one_bone_spine_row_fails_loudly():
    bones = _bones(["Bip01 Spine1", "Bone99"], [-1, 0])
    blob = _model([(1, -1, 0.0, 0.0, 0.9, 0.3, 30.0)])
    with pytest.raises(ValueError, match="unclassified one-bone spine chain"):
        motion.anim_dynamics_breast_bodies(LILY, blob, bones)


def test_vv_anonymous_pair():
    bones = _bones(
        ["Bip01 Spine1", "bone01", "bone03"],
        [-1, 0, 0])
    blob = _model([
