...
class TextureDecodeTests(unittest.TestCase):
    def test_every_admitted_format_builds_one_complete_model(self):
        for fmt in FORMATS:
            with self.subTest(fmt=fmt):
                ...
                self.assertEqual(model.omissions[0]["role"], "low-res-cpu-sample")
                self.assertNotIn("sourceSrgb", model.sampling)
                self.assertNotIn("normal", model.sampling)
                self.assertIn("pointSample", model.sampling)
    ...
    def test_inline_and_external_mips_reconstruct_largest_first(self):
        ...
        self.assertEqual([(level.width, level.height) for level in model.levels], [(8, 8), (4, 4), (2, 2)])
        self.assertEqual(len(model.byte_coverage), 2)

    def test_cubemap_keeps_six_faces_and_ledgers_the_low_end_spheremap(self):
        model = decode_texture(_closure(13, cubemap=True))
        self.assertEqual(len(model.levels[0].images), 6)
        self.assertEqual([face["sourceFace"] for face in model.faces], [0, 1, 4, 5, 3, 2])
        self.assertEqual(model.omissions[-1]["role"], "low-end-spheremap")
        states = model.byte_coverage[0]["stateBytes"]
        self.assertGreater(states["omitted-proven"], 128)
    ...
    def test_cubemap_rotation_preserves_uncompressed_texels(self):
        ...
        self.assertEqual(rotated, bytes((2, 2, 2, 0, 0, 0, 3, 3, 3, 1, 1, 1)))
...
class TextureGlbWriterTests(unittest.TestCase):
    def test_complete_texture_writes_one_ktx2_payload_and_validates(self):
        ...
        self.assertEqual(len(document["bufferViews"]), 1)
        summary = validation.validate_document(document, binary)
        self.assertEqual(summary["mips"], 3)
    ...
    def test_cubemap_ktx2_is_six_faced(self):
        ...
        document, binary = texture_glb.build_document(model)
        summary = validation.validate_document(document, binary)
        self.assertEqual(summary["faces"], 6)
    ...
    def test_public_exporter_publishes_expected_relative_path(self):
        ...
        self.assertEqual(relative, "synthetic/texture.glb")
        self.assertEqual(summary["asset"], "vtmb:texture:synthetic/texture")
    ...
    def test_prepublication_validation_rechecks_source_hashes(self):
        ...
        with self.assertRaisesRegex(validation.TextureGlbValidationError, "source bytes disagree"):
