...
def test_cli_paths_override_environment_and_local_file() -> None:
...
def test_environment_overrides_local_file_and_export_override_is_independent() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        repo = root / "repo"
        repo.mkdir()
...
def test_local_file_is_used_when_arguments_and_environment_are_absent() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary)
        repo = root / "repo"
        repo.mkdir()
