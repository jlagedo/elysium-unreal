...
def _write(directory, blob, name="body.eskm"):
...
def test_returns_the_named_payloads_verbatim(tmp_path):
...
def test_an_absent_section_maps_to_none(tmp_path):
    path = _write(tmp_path, container_bytes({b"SKEL": b"only section"}))
    out = eskm.read_sections(path, (b"SKEL", b"MATL"))
    assert out == {b"SKEL": b"only section", b"MATL": None}


def test_agrees_with_section_digest_about_the_bytes(tmp_path):
    path = _write(tmp_path, container_bytes(SECTIONS))
...
def test_a_foreign_container_raises(tmp_path):
    path = _write(tmp_path, container_bytes(SECTIONS, magic=b"NOPE"))
    with pytest.raises(ValueError, match="not an .eskm container"):
        eskm.read_sections(path, (b"SKEL",))


def test_a_stale_version_raises(tmp_path):
    path = _write(tmp_path, container_bytes(SECTIONS, version=eskm.VERSION + 1))
    with pytest.raises(ValueError, match="re-export"):
        eskm.read_sections(path, (b"SKEL",))


def test_a_truncated_header_raises(tmp_path):
    path = _write(tmp_path, container_bytes(SECTIONS)[:10])
    with pytest.raises(ValueError, match="truncated .eskm header"):
        eskm.read_sections(path, (b"SKEL",))


def test_a_truncated_directory_raises(tmp_path):
    path = _write(tmp_path, container_bytes(SECTIONS)[:16 + 20 * 3 - 4])
    with pytest.raises(ValueError, match="truncated .eskm section directory"):
        eskm.read_sections(path, (b"SKEL",))


def test_a_truncated_section_raises(tmp_path):
    path = _write(tmp_path, container_bytes(SECTIONS)[:-100])
...
def test_a_missing_file_raises_oserror(tmp_path):
    with pytest.raises(FileNotFoundError):
        eskm.read_sections(os.path.join(tmp_path, "absent.eskm"), (b"SKEL",))


def test_section_digest_error_shape_is_unchanged(tmp_path):
    ...
    missing = eskm.section_digest(os.path.join(tmp_path, "absent.eskm"), (b"SKEL",))
    assert missing == "missing:FileNotFoundError"
    foreign = _write(tmp_path, container_bytes(SECTIONS, magic=b"NOPE"), name="foreign.eskm")
