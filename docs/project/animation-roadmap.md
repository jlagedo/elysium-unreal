...
| `Flags & 0x2` split inheritance, ancestors masked out | resolved at bake against the declaring host, which supplies the chain the mask excludes — the result is an ordinary masked local overlay, so the runtime node is the same `FAnimNode_LayeredBoneBlend` as the row above | ANM1 · CCC10 |
...
| Layer masks containing the split bone | **1 of 4** — the 49-bone upper-body gate, carried by every `*_aim_layer` grid cell, by every ranged bobble/relaxed-move layer, and by the *two-handed* melee (`bushhook`, `sledgehammer`); the 24-bone right-arm mask (one-handed melee only), the 45-bone both-arms throwing-star mask and the 1-bone head mask do not (`docs/vtmb/animation_and_movers.md` §A.4) | Scopes the un-normalizable case to one mask, which is why it gets a representation of its own rather than the frame path getting a rule. The mask follows the weapon's **grip**, not ranged-versus-melee, so a consumer partitioning by weapon kind resolves the two-handed melee wrongly |
