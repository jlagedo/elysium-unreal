...
1. **Two clean halves**:
   - **Offline**: the Python decoders in this repo's `pipeline/` decode VtMB's proprietary
     ...
     PNG/DDS, glTF `.glb`, and plain-text sidecars. `pipeline/src/elysium_pipeline/exporters/UE_bsp_to_scene.py` is the exporter;
     `pipeline/src/elysium_pipeline/exporters/export_all.py` batches it.
   - **Offline, second stage**: `dev/elysium.ps1 bake` → `pipeline/unreal/bake_map.py` bakes each exported map's
...
Status lives only in `docs/project/roadmap.md`; current code inventories live beside the code in
`/Source/ElysiumUE/CLAUDE.md`, `/pipeline/CLAUDE.md`, and `/Content/CLAUDE.md`.
...
local `.uasset` products are ignored and regenerated by `dev/elysium.ps1 content`.
...
and status live only in `docs/project/roadmap.md`; its traceability table resolves these names to phases.
...
- **M2 — dressing parity**: props via ISM (+ convex collision), decals, Single Layer
  Water, coronas, checked against the `dev/elysium.ps1 shots` baseline on tutorial + hub maps.
