...
- [x] **[SC1 Scripted camera — the client tracker's numerics and the frame contract](camera_scripted.md#sc1--the-client-trackers-numerics-and-the-frame-contract)** — the 1.0° acquire band, the 1.0 u/s floor, the one-shot snap, the dt clamp/floor, retail's `RemainingTime` arithmetic, the `MoveAccel 0` crawl as an explicit branch, and the FOV copy with its freezing guard.
- [x] **[SC2 Scripted camera — the two channels and the `CInput` composition shape](camera_scripted.md#sc2--the-two-channels-and-the-cinput-composition-shape)** — the cine hard-write split from the blended track override; point lerp, `SimpleSpline` at compose, `roll = e × shotRoll`, the boom skip, mutual exclusion, retail's signed-duration ramp, the off-centre rect, and the release as a **cut with no blend cvar**. Lands with the debug-shots baseline refresh in the same commit.
- [x] **[SC3 Scripted camera — the override fade machinery and the crossfade stack](camera_scripted.md#sc3--the-camera-override-fade-machinery-and-the-crossfade-stack)** — retail's fade state machine verbatim: the signed-duration fields, the lazy reap, the back-dated re-time, per-entity minimums with `dur ≤ 0` snapping, the N-entry stack folded newest-first, and the encoder clamps kept as contract.
- [x] **[SC4 Scripted camera — `camera_cinematic`, the adoption slot, `CamMode` and immobilize](camera_scripted.md#sc4--camera_cinematic-the-director-entity-the-adoption-slot-cammode-and-the-immobilize-pair)** — retires the stub: the director/runtime split, the four targetname anchors, the disposable destroy rule, the **24 Hz goal-publish think in its verbatim order**, all five modes, `point_player`, `m_bDrawPlayer`, the same-tick teardown and the terminal's move onto the slot.
- [x] **[SC5 Scripted camera — shot start, the draw gates and the HUD edge](camera_scripted.md#sc5--shot-start-the-draw-gates-and-the-hud-edge)** — the `End`-without-`Start` live-view dolly-in, the reset-frame/shot-index signal split, the `m_bDrawPlayer` short-circuit, the speed-gated viewmodel, and the HUD hide/show as an **edge** over the port's own HUD.
- [x] **[SC6 Scripted camera — the anchor grammar completed](camera_scripted.md#sc6--the-anchor-grammar-completed)** — `AbsMin`/`AbsMax`, corrected `Top`/`Bottom`, `GrappleVictim`/`GrappleAttacker` on real role state, the `World` fallthrough, the `Follow` offset frame, cached bone indices, the **case-sensitive `AttachType` compare** and the `AttachType None` latch.
- [x] **[SC7 Scripted camera — `AutoPositionFromTarget`](camera_scripted.md#sc7--autopositionfromtarget-and-the-origin-selectors-third-arm)** — the recovered `h = d/sin A`, `r = √(h²+d²)` pull-back, lower-Z point only, suppressed on the third origin-selector arm.
- [ ] **[SC8 Scripted camera — `FindBestShot` and anim events 4050/4051](camera_scripted.md#sc8--findbestshot-and-the-anim-event-channel-4050--4051)** — the `"%s_%d"` enumeration, both geometry predicates, the seeded uniform pick, and the stealth-kill event pair that is `m_bDrawPlayer`'s only writer.
- [ ] **[SC9 Scripted camera — the dialogue and script chain](camera_scripted.md#sc9--the-dialogue-and-script-chain-around-the-camera)** — `SetCamera`'s `DialogDefault` fallback and its absence in `StartPlayerDialog`, the immobilize/holster pair, the payphone arm, the same-tick dialogue-end cut and `DialogPOV`'s feasibility gate. Closes **RE46**.
- [x] **[RG-A Scripted camera recovery — the entity's vtable, shot-start state and unshipped modes](camera_scripted.md#rg-a--the-entitys-own-vtable-shot-start-state-and-unshipped-modes)** — RC2 the `KeyValue` handler and `spawnflags`, RC3 the `+0x564`/`+0x570`/`+0x588` triple, RC4 `player+0x1d60`'s readers, RC12 `CamMode` 3/4, `camera_animated` and `DeathCam`'s driver. Blocks SC4.
- [x] **[RG-B Scripted camera recovery — the track channel's entity interface and publish stack](camera_scripted.md#rg-b--the-track-channels-entity-interface-and-the-publish-stack)** — RC7 `CCameraTrack`/`CCameraKeyFrame` and `vfunc0xBC`–`0xD4` plus the kind-1 pusher, RC8 `asm 10352120`'s fold registers and the `+0x1f44`/`+0x1f48` FOV-vs-roll disagreement. Blocks SC3.
- [x] **[RG-C Scripted camera recovery — the image constants and the client-mode object](camera_scripted.md#rg-c--the-image-constants-and-the-client-mode-object)** — RC9 `_DAT_101e34f4`, `DAT_102de30c`'s name, `_DAT_10235278` and the off-centre cvar names; RC10 `<DAT_104a57e4>->vfunc38/39/40`. Blocks SC1, SC2.
- [x] **[RG-D Scripted camera recovery — the dialogue admission and the gaze gate](camera_scripted.md#rg-d--the-dialogue-admission-and-the-gaze-gate)** — RC5 `FUN_10325da0`, RC6 `FUN_10178120`/`FUN_100e05f0`, the payphone arm and the Python dialogue-advance path. Blocks SC9.
- [x] **[RG-E Scripted camera recovery — the anchor kernels and the grapple role](camera_scripted.md#rg-e--the-anchor-kernels-and-the-grapple-role)** — RC1 `FUN_1013c940`'s clamp, RC11 the client parser `FUN_10028a10` and the client `m_bDrawPlayer` half, RC13 the `+0x1538`/`+0x153c` writers. Blocks SC6, SC7.
...
- [~] **[13.4 Computer terminals & tutorial hacking](plans/terminals.md)** — the parser,
  ...
  (2026-09-07); the cues, the keyboard screen and the HUD hint landed (2026-09-07); email, the cell painter, the diagnostic and the native gym beat landed (2026-09-07); the screen
  material and the Terminus faces authored; open: the Play-tier shot baselines and beat script
  (11.10). Gamepad is out of scope by owner call.
...
| RE46 | the dialogue opener and camera boundary; the server chain is recovered (`StartPlayerDialog` `0x10178280`, `SetCamera` `0x1017d020`, `EndPlayerDialog` `0x10178400`, `DialogPOV`'s single reader) and open only on RG-D — `FUN_10325da0`'s gaze gate and `FUN_10178120`/`FUN_100e05f0` | `docs/vtmb/camera-view-modes.md`; 11.13f, [SC9](camera_scripted.md#sc9--the-dialogue-and-script-chain-around-the-camera), [RG-D](camera_scripted.md#rg-d--the-dialogue-admission-and-the-gaze-gate) | [~] |
