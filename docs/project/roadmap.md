...
- [ ] **[SC7 Scripted camera — `AutoPositionFromTarget`](camera_scripted.md#sc7--autopositionfromtarget-and-the-origin-selectors-third-arm)** — the recovered `h = d/sin A`, `r = √(h²+d²)` pull-back, lower-Z point only, suppressed on the third origin-selector arm.
...
- [ ] **[RG-C Scripted camera recovery — the image constants and the client-mode object](camera_scripted.md#rg-c--the-image-constants-and-the-client-mode-object)** — RC9 `_DAT_101e34f4`, `DAT_102de30c`'s name, `_DAT_10235278` and the off-centre cvar names; RC10 `<DAT_104a57e4>->vfunc38/39/40`. Blocks SC1, SC2.
- [ ] **[RG-D Scripted camera recovery — the dialogue admission and the gaze gate](camera_scripted.md#rg-d--the-dialogue-admission-and-the-gaze-gate)** — RC5 `FUN_10325da0`, RC6 `FUN_10178120`/`FUN_100e05f0`, the payphone arm and the Python dialogue-advance path. Blocks SC9.
- [ ] **[RG-E Scripted camera recovery — the anchor kernels and the grapple role](camera_scripted.md#rg-e--the-anchor-kernels-and-the-grapple-role)** — RC1 `FUN_1013c940`'s clamp, RC11 the client parser `FUN_10028a10` and the client `m_bDrawPlayer` half, RC13 the `+0x1538`/`+0x153c` writers. Blocks SC6, SC7.
