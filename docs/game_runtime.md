...
when something fires them, through the paths in `docs/python_bridge.md` (output
...
is 13** (`savegame_format.md`), so the file's slot space is wider than the sheet's.
...
`"Normal"` dice weighting table (where `dice-system.md`'s d10 resolver plugs in):
...
- ~~Feat roll math — how `Base0 + Base1` forms the dice pool.~~ **Resolved (RE24):** the base list
  ...
  `dice-system.md`; what is settled here is that the feat resolves to an index into that table.
