...
| `FElysiumIOEvent` | struct | queue | `{FireTime, TargetName/TargetHandle, Input, Param, PythonSrc, Activator, Caller, Wire}` — one queued delivery. `Wire` is the authored-wire identity (source entity index, output name, row ordinal; unset for non-wire records), carried for per-wire accounting and serialized with the record so post-restore deliveries still attribute. |
