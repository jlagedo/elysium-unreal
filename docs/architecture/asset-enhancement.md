...
`$ELYSIUM_EXPORT_ROOT/`) and keeps the calibration honest (the calibration defines the *target look*;
...
- **Offline:** a new stage produces a parallel texture set under `$ELYSIUM_EXPORT_ROOT/<map>/tex_hi/` (sky
...
`docs/architecture/rendering-perf.md` → "Floor reality" — read those for the current numbers. Enhancement is
...
Enhanced outputs are game-derived, so they remain under gitignored, regenerable `$ELYSIUM_EXPORT_ROOT/`.
The general bring-your-own-game rule and rationale are `docs/project/rebuild-strategy.md`.
...
`docs/architecture/rendering-perf.md`, movement by `docs/vtmb/source_movement.md`, and audio by `docs/vtmb/audio_pipeline.md`.
