...
because a query is the only place engine knowledge can reach a rule, so it is the only place a rule
can quietly stop being VtMB's.
...
- **Its headless default is part of the contract, not a placeholder.** A `-nullrhi` run and an editor
...
**The decidable test is whether the VtMB fact is authored or observed.** A value someone typed — a
keyfield, a table row, a threshold, a weight, a selection order — is a decision, and it is reproduced
in the substrate however Unreal would have modelled it. A fact only the live world can answer is a
...
- **S11 — A service answers; the substrate decides.** Every call across `FElysiumWorldServices` is an
  ...
  query returns geometry or a candidate and never a verdict, states its headless default, and names
  any divergence from the oracle VtMB used. Authored facts are decisions and stay in the substrate;
  a query approximated with substrate arithmetic is a missing seam, not a rule (§7).
