...
surface properties, sound groups and the script surface — live in `docs/vtmb/audio_pipeline.md`. Status
and implementation order live only in `docs/project/roadmap.md`.
...
The Unreal Audio Mixer owns the rendered graph. Game-agnostic routing packages are generated
locally under `Content/`; game-derived sound waves remain loose intermediates.
...
generated local Unreal reverb/submix presets through a data table; preset `0` is neutral. The exact VtMB
precedence between brush `room_type`, scheme `RoomDSP` and the player's networked room fields is an
RE gate in `docs/vtmb/audio_pipeline.md`; the resolver has separate inputs so that result does not require an
architecture change.
...
`research/tooling/probes/audio_surface_survey.py` and fail new unresolved map wires or missing catalog entries. Live
