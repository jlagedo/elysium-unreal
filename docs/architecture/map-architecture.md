...
`Content/ElysiumGenerated/Boot.umap` is only the boot shell. The generated level carries the map's baked
...
`Content/ElysiumGenerated/Boot.umap`, generated by `pipeline/unreal/make_boot_map.py`, exists so Unreal can create the
