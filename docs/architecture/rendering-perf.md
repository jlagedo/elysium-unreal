...
`research/tooling/probes/probe_light_calibration.py` fits our dynamic-light model against VtMB's own baked
...
JSON summary, and exits. `pipeline/src/elysium_pipeline/validation/profile_report.py` turns the CSVs into the per-pass table
(`$ELYSIUM_EXPORT_ROOT/_profile/<map>_report.md`) and the committed baseline below ("Profiling
baseline").
