...
`BaseTexture` (T2D, `/Engine/EngineResources/DefaultTexture`, colour) and `UseVertexColor` (`#`,
`false`). Nothing else. `DecalDepthOffset` is **not** on this master — it is a knob (below), and
...
| `$spriterendermode` | Source name | Program | `BlendMode` | Units |
|---|---|---|---|---|
| 0 (or the key absent) | `kRenderNormal` | `SpriteRenderNormal` | `BLEND_Opaque` | 60 |
| 1, 2, 3, 4, 9 | `kRenderTransColor`/`TransTexture`/`Glow`/`TransAlpha`/`None` | `SpriteRenderTransColor` | `BLEND_Translucent` | 0 |
| 8 | `kRenderTransAlphaAdd` | `SpriteRenderTransColor` | `BLEND_Additive` | **2** |
| 5, 7 | `kRenderTransAdd` | `SpriteRenderTransAdd` | `BLEND_Additive` | 0 |
| 6 | `kRenderEnvironmental` | none — the shader warns *"Unknown sprite rendermode"* | **stage failure** | 0 |

Only 2 of the 62 declare the key at all, both mode 8. Base Color → Emissive, `Color` × vertex
colour, blend from the row.
...
`$vertexalpha` 731, `$decal` 559 *(550 non-zero — no longer a master switch: it becomes the
provenance flag `isDecalSurface` the placement lane reads, since `IsDecalSurface` had no pin to
gate; 483 `lightmappedgeneric`, 34 `unlitgeneric`, 12 `vertexlitgeneric`, 21 no-master)*,
