...
| **K1 is settled** — the face→axis binding, per-face basis and texcoord flip are read out of `engine.dll`'s own tables and confirmed on the decoded faces (see below) | RE-A1, `research/tooling/probes/probe_sky_orientation.py` |
...
| Gamma is frame-wide, never per-material: `gamma` 2.2, `texgamma` 2.2, `brightness` 0, `linearFrameBuffer` 0, and a display value of `1.6 − clamp(cl_v_gamma − 1, 0, 3)·0.5` (= 1.35 at the default `cl_v_gamma` 1.5), applied as a device gamma ramp at present | RE-A9, `docs/vtmb/color_gamma.md` |
...
| Sky faces live at `materials/skybox/<skyname>{up,dn,lf,rt,ft,bk}` in the standard TTH/TTZ pair | `pipeline/src/elysium_pipeline/exporters/UE_bsp_to_scene.py` |
...
| `TOOLS/toolsskybox` faces mark where the engine draws sky; the exporter skips them | `pipeline/CLAUDE.md` |
...
(`docs/vtmb/light-attribution.md` → "Where to pick up"). The sky/ambience model and the fill
...
`research/tooling/probes/probe_sky_orientation.py` verifies the *relative* half of the convention on the real
...
for `dn`. `research/tooling/probes/sky_probe.py` authors six labelled faces and installs them as a loose set;
`pipeline/src/elysium_pipeline/formats/tex_from_png.py` encodes them into the engine's own container (VtMB has no loose
...
(`docs/vtmb/python_bridge.md`), so `print FindPlayer().GetAngles()` reports `(pitch, yaw, roll)` — of
...
the sample the fill-vs-fixture survey draws from (`docs/vtmb/light-attribution.md`) — 60 of
...
Four independent lines, three of them data. `research/tooling/probes/probe_daynight.py` is the instrument for
...
the BSPs alone (no export, no engine run) with `research/tooling/probes/probe_sky_inventory.py`, measured
...
correction — and its effect on the fill-vs-fixture sample in `docs/vtmb/light-attribution.md` — is
...
wrote $ELYSIUM_EXPORT_ROOT\_sky\inventory.json (108 maps)
...
a data argument end to end. The instrument is `research/tooling/probes/probe_skyambient.py`.
...
- **`docs/vtmb/light-attribution.md` gets a cross-check for free.** The transfer law converts any
...
The world-side shader derivation is canonical in `docs/vtmb/color_gamma.md`. The sky-specific comparison
...
`docs/vtmb/color_gamma.md`. Sky, world, and models share that path; there is no per-sky gamma stage.
...
- **Lump 8** — the authored ambience, and the only bake there is (K4); decoded by
  `research/tooling/probes/lightmap.py`, and `probe_light_calibration.py` already fits against it.
...
- **`research/tooling/probes/sky_probe.py`** + **`pipeline/src/elysium_pipeline/formats/tex_from_png.py`** — the labelled-sky set and the
...
- **`research/tooling/probes/probe_sky_inventory.py`** — the K8 instrument (RE-A7): per map, the `skyname` and
  ...
  tables, and it writes `$ELYSIUM_EXPORT_ROOT/_sky/inventory.json`.
- **`research/tooling/probes/probe_daynight.py`** — the K4 instrument (RE-A4): over every map in the install, the
...
- **`research/tooling/probes/probe_skyambient.py`** — the K6 instrument (RE-A5), in four modes.
  ...
  `$ELYSIUM_EXPORT_ROOT/_skyambient/<map>.json`. It refuses a patch-recompiled bake unless asked, which is
...
- **Ghidra** — `research/tooling/ghidra/driver/run.ps1`; targets `engine.dll` (sky draw, fog, day/night),
