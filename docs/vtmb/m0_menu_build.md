...
> **Partly superseded — read `docs/vtmb/vtmb-ui.md` first.** This doc decompiles `GameUI.dll`'s
...
> constants (§7). `docs/vtmb/vtmb-ui.md` §6 tabulates each correction. Still valid here: the `.fnt` format and face inventory (§3), the TrackerScheme colour
...
`pipeline/src/elysium_pipeline/formats/fnt.py` — header `u32[9]`; `@36` 256-byte char→glyph map; `@292` glyphCount ×
...
Tooling + dumps under `$ELYSIUM_WORK_ROOT/research/ghidra/` (local-only; the whole tree is gitignored, never committed).
...
The shipped main/pause menu inventory is canonical in `docs/vtmb/vtmb-ui.md` §2. This section remains
