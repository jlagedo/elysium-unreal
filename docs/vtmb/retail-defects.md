...
load-bearing: it is why the dialogue camera re-resolves its anchors every tick. Four came from the
RG-A pass (`+0x564` triple, `m_iVFlags`, the unshipped `CamMode` arms), of which the `point_player`
row is reachable on every shipped shot; the `npcfollowmove.txt` row is an **authoring** defect the
parser absorbs silently, and it moves a shipped shot by 152 cm. The last row — the
`camera_animated` strand — was appended 2026-09-07 from the RG-F pass
(`$ELYSIUM_WORK_ROOT/_camera_recovery/rc_group_f.md` §RC15.3).

| Defect | Evidence | Bearing |
|---|---|---|
| **`RemainingTime` NaNs its own pan.** `FUN_100010f0` (`client.dll` `0x100010f0`–`0x100011bb`), the remaining-translation-time solve `SyncRotateOnMove` divides the angular delta by, has two arithmetic faults: the trapezoid arm's acceleration distance is `(vmax − v)²/2a` instead of `(vmax² − v²)/2a`, correct only from `v = 0`; and the triangle arm's radicand `2a − 0.5(d − R2)` is an acceleration minus a distance and goes **negative** whenever `d − R2 > 4a`, so `sqrtf` returns NaN and the turn rate with it. At the shot-parse defaults (`MoveSpeed 150`, `MoveAccel 50`) the triangle arm is entered for `d ≤ 450` and the radicand is negative past `d − R2 > 200`, so a default-rate `SyncRotateOnMove` shot travelling 200–450 units freezes its pan. **[corpus]** | the band needs `MoveSpeed > 2·MoveAccel`; none of the 32 shipped `SyncRotateOnMove` shots has that (all author their own rates — `jack.txt` 500/250, `special-case.txt` 300/450, `andreibasement.txt` 80/40), so it is reachable only from the parse defaults, which no such shot uses | **Named modernization M6 — the triangle arm's radicand is clamped at zero, and nothing else.** The trapezoid defect is kept: it is the timing all 33 shipped `SyncRotateOnMove` shots are tuned against — `jack.txt` closing 100 u from rest answers 0.17 s under it, against 1.27 s (a ~7× slower pan) from a correct kinematic solve. The NaN state needs `MoveSpeed > 2·MoveAccel`, which no shipped shot has, so no shipped pan changes. The distance the solve divides is `m_flDistRemaining` (`+0x4c4`), which the position step stores **before** it moves and the turn solve reads the same frame — measuring it after the step feeds the defect a distance one step short every frame |
| **`Point2` without `Point1` aims at the origin.** The shot parser `FUN_100721e0` (`vampire.dll`) raises the target-point flag by *order of presence*, `flags \|= 1 << (count + 2)`, while writing the anchor into the fixed Point1/Point2 slot. A file authoring only `Point2` fills slot 3 but raises `0x04` (Point1), and the look-at solve `FUN_1006f670` then reads the empty slot 2 — a shot that looks at `(0,0,0)`. **[corpus]** | no shipped file authors `Point2` alone | Unreachable in the corpus. The client parser `client.dll FUN_10028d20` has the identical bug (RC11) |
| **The anchor cache reads anchor 0's flags for every anchor.** `FUN_1006f010` (`vampire.dll`) decides whether to return the shot-start cache or resolve live by testing `AttachType None` (`0x2000`) — but it passes the literal index `0` to the flag accessor, so every one of the four anchors is judged by the **Start** anchor's flags. A shot with no `Start` block reads a zeroed record, so `0x2000` is clear and all four anchors re-resolve live every tick. **[corpus]** | every shipped dialogue shot: `jack.txt` and `dialogdefault.txt` have no `Start` block, so retail re-resolves their `Bone: Bip01 Head` target every tick and the authored `AngularTolerance [10, 10, 10]` is what holds the camera still | Retail's single term is `anchor[0].flags & 0x2000`, equivalently "`Start` present **and** its `AttachType` is `None`" — the two agree because a shot with no `Start` leaves anchor 0 zeroed. The anchor-0 test is what decides which shots latch: 55 of the 66 shipped files behave differently without it. `special-case.txt`'s `Follow` authors `Start { AttachType None }` and is the one shipped shot that latches |
| **`point_player` is authored on the director and read on the camera.** `m_bForcePlayerLook` (`+0x5e8`, key `point_player`) is initialised to **1** by the `CBaseCineCam` constructor `FUN_1006d620` (`*(byte *)(this + 0x17a) = 1`). `FUN_10070780` copies **only** `m_bDrawPlayer` (`+0x640`) from the director onto the runtime camera, and `FUN_10070470` / `CBasePlayer::SetCamera` / `StartPlayerDialog` copy nothing at all; a map-placed director is never adopted, so its own `+0x5e8` is never read. The keyvalue is parsed into a field the shot never consults. **[corpus]** | 35 of 51 shipped `camera_cinematic` entities author it — **26 of them `0`** — and all 35 values are discarded. Only `CFuncMonitor::vfunc39` `0x10115260`, `CBasePlayer::HandleAnimEvent` 4050 `0x10178a10` and the `"Intrusion"` opener `FUN_10225070` clear it, each on the camera it just created | The runtime camera defaults to *forcing* the subject's gaze at the look-at every tick (`FUN_1006f8f0` tail → `FUN_10178590` → `FUN_10178550`); the director's `point_player` is parsed and dead, and only those three openers opt out |
| **`m_iVFlags` bits `0x2` and `0x4` are read and cleared but never set.** `CBasePlayer::SetAnimation` (slot 417, `0x10164870`) selects activity `0xfa3` ("Grapple Release") on bit `0x2` and `0xfc9` ("Seductive Release") on bit `0x4`, then runs `RemoveVFlags(6)`. That `RemoveVFlags(6)` is the only writer of either bit anywhere in `vampire.dll`: `AddVFlags` `0x10181580` is called with `1` and `8` and nothing else, and `ToggleVFlags` `0x10181620` / `ClearVFlags` `0x10181600` have no callers. **[corpus]** | both release animations are unreachable in the shipped build | **Ruling — reproduce the consumer, record the gap:** the two release requests stand, and no shipped path raises either bit |
| **`CamMode 3` null-dereferences on anchor loss.** `FUN_1006fe00` (`vampire.dll`) opens with `if (!handleLive(+0x610)) this->+0x55c = 0;` — but the expiry gate in `FUN_1006f7d0` is `+0x55c > 0.0f && +0x55c < curtime`, which `0` fails, so the think does not end. It then resolves the same handle with a weaker test (index and serial only, no entry-pointer check), gets `NULL`, and calls `ent->WorldSpaceCenter()` (`vfunc 0x300`) on it. **[corpus]** | unreachable: the mode-3 factory `FUN_100705d0` has **zero callers** in the image, so no shipped path ever creates a `FollowEntity` camera | **Ruling — reproduce the no-expiry, drop the crash:** a dead anchor does not end the shot (the camera freezes on its last pose); the null dereference is not reproduced |
| **`CamMode 4`'s per-tick FOV publish is dead.** `FUN_1006f870` publishes `m_flFOV = shot->FieldOfView` every server tick, but `C_BaseCineCamera::Update` (`client.dll` `FUN_10001a20`) takes `if (m_CamMode == 4) { FUN_10002200(dt); return; }` and `FUN_10002200` is an empty `RET`; the copy-through that would move `m_flFOV` into `m_flCurFov` lives in the fall-through arm mode 4 never reaches, and shot start `FUN_10002210` never writes `m_flCurFov` either. So an `Animated` camera renders pose *and* FOV frozen at shot start. **[corpus]** | zero `camera_animated` entities ship, though `special-case.txt` authors the `Animated` shot for it with a "don't change this" comment | **Ruling — reproduce as written**, including the publish that nothing consumes |
| **Both crossfade pushers write the kind byte `0`, so the fold's target arm is unreachable.** `CBasePlayer::SetCameraViewEntity` `FUN_1017d280` at `0x1017d386` and the target setter `FUN_1017d460` at `0x1017d55f` are both `MOV byte ptr [ESI],0x0`, while `CHL2_Player::SetupVisibility` discriminates on that byte (`0x10352485 TEST DL,DL ; JNZ 0x1035259e`). A superseded **target** camera is therefore folded through the **view** arm: its `vfunc0xC8` viewpoint, `vfunc0xC0` roll and `vfunc0xC4` FOV are crossfaded into the published *view* override and it consumes `w[0]`, the view channel's coverage, while the target point itself simply snaps to the new target. The fade list's kind-1 branch (`0x1035259e`-`0x1035261f`) never executes in a shipped run. **[corpus]** | reachable wherever a map supersedes a camera target: `SetHeadAsCameraTarget` / `SetBodyAsCameraTarget` / `FadeHead…` / `FadeBody…` fire on every `CBaseCombatCharacter` (`sm_hub_1` fires `!playercontroller.SetBodyAsCameraTarget`), and because `CBaseCombatCharacter` overrides neither slot 48 nor slot 49 the drag is toward the `CBaseEntity` defaults — FOV **75.0** and roll **0.0** | **Ruling — reproduce, kind-1 arm kept dead-but-present** so the shape stays retail's. A superseded target drags the published view origin and pulls the FOV toward 75 |
| **`npcfollowmove.txt` loses two thirds of its own offset.** The file authors `"OffsetOrigin"  [-60, 0, 72]` **unquoted** on both its `Start` and its `End`. A KeyValues bare run ends at the first whitespace, so the value handed to the bracket parser `FUN_10071cd0` (`vampire.dll`) is the token `[-60,`; the parser's three ordered scans then answer `x = atof("-60,") = -60`, `y = 0` (the comma is the last character and the remainder is empty) and `z = 0` (no third separator). The authored `72` u of height never reaches the shot | `$ELYSIUM_WORK_ROOT/exports/vdata/camerashots/npcfollowmove.txt:12`, `:20`; `FUN_10071cd0` `0x10071d04`/`0x10071d36`/`0x10071d68`, separators `0x10547308 = "["` and `0x10547304 = ","` | Retail's three ordered scans run over the running remainder, so both anchors carry `(-60, 0, 0)` u — 60 u (152.4 cm) behind the eye and level with it, which is the framing the shipped shot actually has. A parse that rejects the truncated token instead drops the offset entirely and frames the shot 152 cm away from retail |
| **A `camera_animated` with an unresolvable `animname` strands the view permanently.** `FUN_10071770`'s `seq < 0` arm prints `Msg("%s no sequence named:%s\n")`, writes `m_nSequence = 0` — **not** `-1` — and returns. It fires no `OnCameraBegin`, calls no `ThinkSet` and never touches `m_flNextThink`, so the 10 Hz think never runs and `EndCamera` `FUN_10071660` — the only exit — is never reached. The `CamMode 4` camera `StartCamera` `0x10071550` created and adopted two lines earlier stays adopted, frozen at sequence 0 cycle 0; `OnCameraComplete` never fires; and the `spawnflags & 1` immobilize, applied immediately *after* the failed call, is never released. The player is frozen looking at a still camera until something else takes the adoption slot. The `Msg` is itself defective: one argument pushed for two `%s`, so the missing sequence name is never printed. **[corpus]** | `FUN_10071770` (the arm at the `LookupSequence` return test), `FUN_10071550`'s `if (m_spawnflags & 1) SetImmobilized(player, true)` after it, `FUN_10071660` reachable only from the think and from `InputEndCamera`; unreachable in shipped content — **zero `camera_animated` entities across 108 `.ents`**, while `special-case.txt` authors the `Animated` shot for the class with Troika's "don't change this" comment | **Owner ruling 2026-09-07 — default to retail:** the strand is reproduced, not repaired; an earlier "end the camera on the next think" divergence is withdrawn. A map-wired `EndCamera` input is still the way out, because the strand is "nothing fires the exit", not "the exit is broken" |
