...
(`docs/contracts/seam_map_map.md` → "Section keys: `#underside` and `#style<n>`", verdict B2),
never off which unit a `$bottommaterial` value happens to name — the R7.1/R7.4 rule that read it
off self-reference is retired (`docs/contracts/seam_map_material.md` → `M_V2_Water` → "The
