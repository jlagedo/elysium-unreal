...
FOV) live in `source_movement.md`, and the `+use` look-cursor / `use_icon` machinery lives in
`entity_io.md`. This doc owns the **binding layer**.
...
- `cfg/*.cfg` — `default.cfg`, `config.cfg`, `user.cfg`, `autoexec.cfg`, `valve.rc`
  (mirrored verbatim to `tools/out/cfg/` by `tools/UE_extract_cfg.py`).
...
- Decompiles and byte-scans of `Bin/engine.dll`, `Vampire/cl_dlls/client.dll`,
  ...
  `tools/ghidra/run.ps1 -Script DumpGrep`.
...
| ±`reload` | reload |
| ±`use` | the world-interaction verb (see `entity_io.md` for the trace, `use_icon`, `PassesUseFilter`) |
...
| `cfg/user.cfg` | user/mod overrides, exec'd *after* `config.cfg` so it wins. The Unofficial Patch's whole alias vocabulary lives here, including the Basic/Plus switch `alias patchtype "setPlus()"` (see `python_bridge.md`). |
...
`m_pitch × sensitivity` = 0.066 (see `source_movement.md`). The patch only moves the widgets
