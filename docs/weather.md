...
`roadmap.md` — task **7.9** (the runtime), **PL12** (mirror the particle definitions),
**RE23** (close the format and the wetness semantics).

Related: `sky-ambience.md` (the 3D skybox miniature the rain draws inside), `entity_io.md`
(the I/O bus `FadeGlobalWetness` arrives on), `audio_pipeline.md` (the SoundScheme layer),
`rendering-perf.md` (the translucency budget this has to fit).
...
2. **Who calls it.** The level scripts fire it, but the call sites are not yet surveyed against
   the exported `out/scripts/`, so we cannot say whether wetness tracks story beats or is set
   once per map.
...
Five systems, not one. Per `remaster-direction.md` the split is clean: the **appearance** is
...
2. **Occlusion.** Rain falling through roofs is the tell. The maps are static and small and the
   ...
   `out/<map>/<map>.obj` rather than captured at runtime — `worldspawn`'s `world_mins`/`world_maxs`
...
`profile.bat` and `shots.bat` already cover the outdoor vantages, so this is measurable.
