...
- [ ] **3.7 Grade/tonemapper fidelity** *(was L2.3 + M1 polish)* — stop the filmic curve
  ...
  `elysium.*` A/B toggles for sky/LUT/fog. Sky orientation moved out to `sky-ambience.md`
  Phase B (K1 settled by RE10, K2 out of UE's own source — both halves of the transform are
  stated, so the fix is B3). *Deps:* 3.6.
