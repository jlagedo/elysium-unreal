...
DEFAULT_DEPTH = 6
DEFAULT_PACK = 120
PACK_BUDGET = 70_000   # characters of decompiled C per reading pack
...
# The project source `gen_kernel_shape` writes from these very tables. The citation scan skips it;
# see `Ledger.citations`.
GENERATED_CENSUS = ("ElysiumNpcKernelShape.cpp", "ElysiumNpcKernelSlots.inl",
                    "ElysiumNpcKernelSlots.cpp", "ElysiumNpcKernelTunables.h",
                    "ElysiumNpcKernelTunables.cpp")
...
VERDICT_WORDS = ("rule", "mechanism", "present", "dead")
...
@dataclass
class Verdict:
    """One overlay row: what a porting story decided about one closure function.

    The verdict is the story's *citation* of the function — the row says the body was read and
    what was done about it — so it lands here rather than in the generated checklist, which is
    rewritten from the corpus on every run.
    """
...
@dataclass
class Tunable:
    """One overlay row: a retail number, where it lives, its width and the value the image holds."""
...
def load_tunables(path: Path) -> list[Tunable]:
    """The tunables overlay in file order. A malformed row is fatal: this table is a record."""
    rows: list[Tunable] = []
    if not path.is_file():
        return rows
    header = False
    ...
    for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if not header:
            header = True
            if tuple(cells) != TUNABLE_COLUMNS:
                raise SystemExit(f"{path}:{number}: the first non-comment row must be the header "
                                 f"{'/'.join(TUNABLE_COLUMNS)}")
            continue
        if len(cells) != len(TUNABLE_COLUMNS):
            raise SystemExit(f"{path}:{number}: {len(cells)} cells, expected "
                             f"{len(TUNABLE_COLUMNS)} ({'/'.join(TUNABLE_COLUMNS)})")
        addr, name, kind, value, evidence = (c.strip() for c in cells)
        addr = addr.lower().removeprefix("0x")
        if not re.fullmatch(r"10[0-9a-f]{6}", addr):
            raise SystemExit(f"{path}:{number}: {addr!r} is not an image address")
        if kind not in TUNABLE_TYPES:
            raise SystemExit(f"{path}:{number}: type {kind!r} is not one of "
                             f"{', '.join(TUNABLE_TYPES)}")
        if not TUNABLE_NAME_RE.match(name):
            raise SystemExit(f"{path}:{number}: name {name!r} is not PascalCase")
        if addr in addresses:
            raise SystemExit(f"{path}:{number}: {addr} already has a row")
        if name in names:
            raise SystemExit(f"{path}:{number}: {name} is already a tunable's name")
        if not value:
            raise SystemExit(f"{path}:{number}: every row needs its value")
        if not evidence:
            raise SystemExit(f"{path}:{number}: every row needs its one-line evidence")
        addresses.add(addr)
        names.add(name)
        rows.append(Tunable(addr, name, kind, value, evidence))
    return rows


def inline_cells(repo: Path, tunables: list[Tunable]) -> dict[str, set[str]]:
    """`DAT_` cells the NPC substrate names that the overlay does not hold, per file."""
    held = {row.address for row in tunables}
    # ConVar rows name the object; readers cite the pointer at object + 4. Both
    # spellings are covered by that row. Ordinary cells have no such adjacent alias.
    held.update(f"{int(row.address, 16) + 4:08x}" for row in tunables
                if row.type.startswith("convar_"))
    found: dict[str, set[str]] = {}
    substrate = repo / "Source" / "ElysiumUE" / "Private" / "Substrate"
    for path in sorted(substrate.glob("ElysiumNpc*")):
        if path.suffix not in (".h", ".cpp", ".inl") or path.name in GENERATED_CENSUS:
            continue
        text = path.read_text(encoding="utf-8", errors="replace")
        cells = {m.group(1).lower() for m in DATA_CELL_RE.finditer(text)} - held
        if cells:
            found[path.name] = cells
    return found


def parse_band(text: str) -> tuple[int, int]:
    """`"0-9"` → `(0, 9)`; a bare `"7"` is the single layer."""
    low, _, high = text.partition("-")
    try:
        return (int(low), int(high if high else low))
    except ValueError as error:
        raise SystemExit(f"kernel_ledger: {text!r} is not a layer band like `0-9`") from error


def load_verdicts(path: Path) -> dict[str, Verdict]:
    """The verdict overlay, keyed by address. A malformed row is fatal: this table is a record."""
    ...
    for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if not header:
            header = True
            if tuple(cells) != VERDICT_COLUMNS:
                raise SystemExit(f"{path}:{number}: the first non-comment row must be the header "
                                 f"{'/'.join(VERDICT_COLUMNS)}")
            continue
        if len(cells) != len(VERDICT_COLUMNS):
            raise SystemExit(f"{path}:{number}: {len(cells)} cells, expected "
                             f"{len(VERDICT_COLUMNS)} ({'/'.join(VERDICT_COLUMNS)})")
        addr, verdict, band, target, evidence = (c.strip() for c in cells)
        addr = addr.lower().removeprefix("0x")
        if verdict not in VERDICT_WORDS and verdict != UNSETTLED_VERDICT:
            raise SystemExit(f"{path}:{number}: verdict {verdict!r} is not one of "
...
class Ledger:
    ...
    def band_core(self, low: int, high: int) -> list[str]:
        """The core functions whose `order.md` layer falls in a band, in build order."""
        return sorted((a for a in self.core() if low <= self.layer_of.get(a, -1) <= high),
                      key=lambda a: (self.layer_of.get(a, -1), a))

    def caller_counts(self) -> dict[str, collections.Counter]:
        counts: dict[str, collections.Counter] = collections.defaultdict(collections.Counter)
        for _, callee, kind in self.closure_edges:
            counts[callee][kind] += 1
        return counts

    def band_stats(self, low: int, high: int) -> dict[str, int]:
        """The per-band measure `## Verdicts by layer band` in `coverage.md` prints.

        A verdict is a citation: the overlay row says the body was read and what was done with it.
        `neither` is the acceptance measure — a core function of the band that the port does not
        cite, the oracle does not walk, and no story has verdicted.
        """
        rows = self.band_core(low, high)
        stats = {"core": len(rows), "verdicted": 0, "unsettled": 0, "empty": 0,
                 "port": 0, "oracle": 0, "neither": 0, "damaged": 0}
        for word in VERDICT_WORDS:
            stats[word] = 0
        for addr in rows:
            verdict = self.verdicts.get(addr)
            if verdict is None:
                stats["empty"] += 1
            else:
                stats[verdict.verdict] += 1
                if verdict.verdict != UNSETTLED_VERDICT:
                    stats["verdicted"] += 1
            if self.functions[addr].damaged:
                stats["damaged"] += 1
            port = bool(self.cites(self.port_addr, addr))
            oracle = bool(self.cites(self.oracle_addr, addr))
            stats["port"] += int(port)
            stats["oracle"] += int(oracle)
            if not port and not oracle and verdict is None:
                stats["neither"] += 1
        return stats

    def _render_checklist(self, band: str) -> str:
        low, high = parse_band(band)
        rows = self.band_core(low, high)
        counts = self.caller_counts()
        stats = self.band_stats(low, high)
        out = self._head(
            f"NPC kernel — porting checklist, layers {low}–{high}",
            "One row per *core* function (a family or helper class method, or a body touching an "
            "offset past `CBaseCombatCharacter`'s layout) whose `order.md` layer falls in this "
            "band, in build order. The **Verdict** column is the porting story's record of what it "
            "did with the body, and it is not stored here: it comes from "
            f"`research/tooling/ghidra/driver/{VERDICTS_TSV}`, so regenerating this table from the "
            "corpus never loses one.\n"
            "\n"
            "- **rule** — the body carries a formula, a threshold, an ordering or a state write a "
            "program can observe. It is ported verbatim onto the port's virtual, cites the retail "
            "address, and has one automation test derived from the decompiled C. *Target* names "
            "the port method; *Evidence* leads with the observable, `[0019/1 obs=<kind>: …]`.\n"
            "- **mechanism** — Unreal already supplies it (a trace, a physics query, an audio "
            "call, a string or CRT routine, a math or container helper). *Target* names the "
            "service seam; nothing is ported.\n"
            "- **present** — the port already runs it. *Target* names the port function, which "
            "cites the retail address, so the Port column below turns true on the next run.\n"
            "- **dead** — nothing can observe it: no keyfield, schedule text, script name, output, "
            "save field, player-visible timing or witness test would notice its absence (spec "
            "0019 story 1). One row, no body, no test; `delete-list.md` lists the port bodies "
            "still standing.\n"
            "- **unsettled** — read and not settled; *Evidence* says why. Counted apart: it is a "
            "recorded failure to reach a verdict, not a verdict.\n"
            "\n"
            "‼ marks a damaged decompilation; those rows are read from `corpus asm <addr>` one at "
            "a time and are never verdicted in bulk.")
        out += [
            "| Measure | Count |", "|---|---|",
            f"| Core functions, layers {low}–{high} | {stats['core']} |",
            *[f"| … `{word}` | {stats[word]} |" for word in VERDICT_WORDS],
            f"| … `{UNSETTLED_VERDICT}` | {stats['unsettled']} |",
            f"| … no verdict yet | {stats['empty']} |",
            f"| … damaged decompilation | {stats['damaged']} |",
            f"| … cited by the port | {stats['port']} |",
            f"| … cited by the oracle | {stats['oracle']} |",
            f"| … cited by neither and unverdicted | {stats['neither']} |",
            "",
            "| Address | Function | Size | Layer | Slots | Writes | Reads | Callers | Verdict "
            "| Target | Evidence |",
            "|---|---|---|---|---|---|---|---|---|---|---|",
        ]
        for addr in rows:
            fn = self.functions[addr]
            slots = self._list_cell(
                [f"`{c}#{s}`" for c, s in sorted(fn.slots) if c in self.family or c in self.helpers],
                3)
            cnt = counts[addr]
            outside = len(self.all_callers.get(addr, ())) - cnt["direct"] - cnt["virtual"]
            call_cell = (f"{cnt['direct']}d/{cnt['virtual']}v/{cnt['slot-candidate']}c"
                         + (f" +{outside} outside" if outside > 0 else ""))
            verdict = self.verdicts.get(addr)
            out.append(
                f"| `{_hex(addr)}` | {fn.label}{' ‼' if fn.damaged else ''} | {fn.size} "
                f"| {self.layer_of.get(addr, '')} | {slots} "
                f"| {self._list_cell([self._off_cell(fn, o) for o in sorted(fn.writes)], 4)} "
                f"| {self._list_cell([self._off_cell(fn, o) for o in sorted(fn.reads)], 4)} "
                f"| {call_cell} "
                f"| {('`' + verdict.verdict + '`') if verdict else ''} "
                f"| {_cell(verdict.target) if verdict else ''} "
                f"| {_cell(verdict.evidence) if verdict else ''} |")
        return "\n".join(out) + "\n"

    def bodies(self, band: str, per_pack: int) -> dict[str, str]:
        """The decompiled C of a band's rows, in packs, for the reading the verdicts come from.

        Never committed — decompilation lives under `ELYSIUM_WORK_ROOT` (`research/CLAUDE.md`).
        Written as one file per pack so a reader opens a pack instead of querying the corpus once
        per row.
        """
        low, high = parse_band(band)
        rows = self.band_core(low, high)
        counts = self.caller_counts()
        packs: dict[str, str] = {}
        # Row count alone makes a useless pack: 120 three-byte accessors are 6 KB and 120 bodies
        # from the top of the band are a megabyte. A pack closes on whichever bound it reaches
        # first, so every pack is one reading.
        chunks: list[list[str]] = [[]]
        budget = 0
        for addr in rows:
            if chunks[-1] and (len(chunks[-1]) >= per_pack or budget >= PACK_BUDGET):
                chunks.append([])
                budget = 0
            chunks[-1].append(addr)
            budget += len(self.functions[addr].code or "")
        for number, chunk in enumerate(chunks, 1):
            first = rows.index(chunk[0]) + 1
            out = [f"# Band {low}-{high}, pack {number} of {len(chunks)} "
                   f"({len(chunk)} functions, rows {first}-{first + len(chunk) - 1} of "
                   f"{len(rows)})", ""]
            for addr in chunk:
                fn = self.functions[addr]
                cnt = counts[addr]
                outside = len(self.all_callers.get(addr, ())) - cnt["direct"] - cnt["virtual"]
                out += [f"## {_hex(addr)}  {fn.label}" + ("  ‼ DAMAGED" if fn.damaged else ""), "",
                        f"- size {fn.size}, layer {self.layer_of.get(addr, '')}, "
                        f"callers {cnt['direct']}d/{cnt['virtual']}v/{cnt['slot-candidate']}c"
                        + (f" +{outside} outside" if outside > 0 else ""),
                        "- slots: " + self._list_cell(
                            [f"{c}#{s}" for c, s in sorted(fn.slots)
                             if c in self.family or c in self.helpers], 10),
                        f"- writes: " + (", ".join(_fmt_off(o) for o in sorted(fn.writes)) or "—"),
                        f"- reads: " + (", ".join(_fmt_off(o) for o in sorted(fn.reads)) or "—"),
                        f"- port cites: {self._cite_cell(self.cites(self.port_addr, addr))}",
                        f"- oracle cites: {self._cite_cell(self.cites(self.oracle_addr, addr))}",
                        ""]
                if fn.damaged:
                    out += [f"- warnings: {'; '.join(fn.damaged)}", ""]
                out += ["```c", (fn.code or "(no decompilation)").rstrip(), "```", ""]
            packs[f"band-{low}-{high}-pack-{number:02d}.md"] = "\n".join(out) + "\n"
        return packs
    ...
    def _render_coverage(self) -> str:
        ...
        out += ["", "## Verdicts by layer band", "",
                ...
                "(`checklist-<band>.md` is the rendered table). A verdict is a citation: the row "
        ...
        out += ["", "## Tunables", "",
                "Spec 0019 story 4. `kernel_tunables.tsv` holds each retail number a kernel body "
                "reads, `gen_kernel_tunables` renders it into `ElysiumNpcKernelTunables.h/.cpp` and "
                "its `--check` reads every row back out of the pinned image at the row's width. "
                "*Outstanding* is the `DAT_` cells the NPC substrate still spells inline; story 6 "
                "moves them family by family (`gen_kernel_tunables --report` lists them).", "",
                "| Type | Rows |", "|---|---|",
                *[f"| `{kind}` | {by_type[kind]} |" for kind in TUNABLE_TYPES if by_type[kind]],
                f"| **All** | **{len(tunables)}** |",
                f"| Outstanding inline cells (files) "
                f"| {len(set().union(*queue.values())) if queue else 0} ({len(queue)}) |"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--checklist", action="append", default=[], metavar="BAND",
                        help=f"also render `checklist-<band>.md` for a layer band like `10-18`; "
                             f"{', '.join(CHECKLIST_BANDS)} are always rendered")
    parser.add_argument("--bodies", metavar="BAND",
                        help="write the band's decompiled bodies as reading packs under "
    ...
    print(f"family {len(ledger.family)} classes, {len(ledger.slot_bodies)} slots, "
          ...
          f"{len(ledger.verdicts)} verdicts")
    if args.bodies:
        packs = ledger.bodies(args.bodies, args.pack)
        pack_dir = research_root() / "npc-kernel-checklist"
        pack_dir.mkdir(parents=True, exist_ok=True)
        for name, text in packs.items():
            (pack_dir / name).write_text(text, encoding="utf-8", newline="\n")
        print(f"wrote {len(packs)} reading packs to {pack_dir}")
