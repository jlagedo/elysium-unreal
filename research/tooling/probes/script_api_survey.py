...
`docs/vtmb/python_bridge.md` establishes *how* the binding works (a datamap reflection layer, not a
...
Reads (all under `$ELYSIUM_EXPORT_ROOT/`, the gitignored regenerable mirror — nothing game-sourced is written):

  * `$ELYSIUM_EXPORT_ROOT/scripts/**/*.py`   — 41 level scripts + `vamputil`/`fileutil`/`zvtool` (PL2)
  * `$ELYSIUM_EXPORT_ROOT/dlg/**/*.dlg`      — 147 dialogues; field 4 (condition, eval) + field 5 (action, exec)
  * `$ELYSIUM_EXPORT_ROOT/*/*.ents`          — the exported maps' output **field 6** Python payloads (PL/1.2)
...
SRC = os.path.join(os.fspath(repo_root()), "Source", "ElysiumUE", "Private")
