...
`docs/vtmb/sky-ambience.md` -> K1 reads the Source sky convention out of `engine.dll`'s own
...
re-export), and those originals are moved into `$ELYSIUM_EXPORT_ROOT/_skyprobe/backup/` on install and put
...
from elysium_pipeline.formats import tex_from_png as tfp
...
WORK = os.fspath(research_root() / "skyprobe")
...
def report(skyname):
    print(f"\nThe prediction (docs/vtmb/sky-ambience.md -> K1). Facing each Source axis, the "
