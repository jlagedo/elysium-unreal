"""Mirror the VtMB UI source -- layouts, schemes, strings and art -- into `out/ui/`.

Roadmap **PL8**. The UI has no classic mode (`docs/remaster-direction.md` axis 1), so this
...
Two schemes are live and they are **not** interchangeable (RE, `docs/vtmb-ui.md`):
...
type. Vector faces live in `Content/Fonts` (`tools/fetch_ui_fonts.py`).
...
Usage:
  python tools/UE_extract_ui.py              # layouts, strings, menu scene, HUD + screen art
  python tools/UE_extract_ui.py --inventory  # also the ~350 item/armor inventory icons
  python tools/UE_extract_ui.py --force      # redo files already present
"""
import argparse
import json
import os
