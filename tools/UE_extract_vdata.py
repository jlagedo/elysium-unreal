"""Copy VtMB's plain-text data tables (`vdata/`) verbatim into out/vdata.
...
consumers are built (per `docs/vdata-catalog.md`, which maps each table to its system + task).

Copied **verbatim** -- no parse, no transcode -- same bring-your-own-game posture as the
script/dialogue/sign mirrors: output lives under out/ (gitignored, regenerable), resolved
...
Two subtrees are deliberately excluded:
  * `vdata/signs/` -- owned by `UE_extract_signs.py` (it also decodes the sign background art
    into out/signs), so mirroring it here would duplicate.
...
Usage:
  python tools/UE_extract_vdata.py            # copy vdata tables
  python tools/UE_extract_vdata.py --force    # re-copy even files already present
"""
import os
import sys
