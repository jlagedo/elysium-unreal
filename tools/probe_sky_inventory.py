...
Usage:
    python tools/probe_sky_inventory.py                 # every map (108), rollup
    python tools/probe_sky_inventory.py sp_tutorial_1 sm_hub_1
    python tools/probe_sky_inventory.py --markdown      # + the doc tables
    python tools/probe_sky_inventory.py --verbose       # + a line per map

Writes `tools/out/_sky/inventory.json` (every measured field, per map) so a later
...
OUT_DIR = Path(__file__).resolve().parent / "out" / "_sky"
SKYPROBE_DIR = Path(__file__).resolve().parent / "out" / "_skyprobe"
