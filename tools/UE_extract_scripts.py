"""Copy the game's loose Python level scripts + dialogue into out/scripts and out/dlg.
...
mirror (`out/scripts/<rel>`, `out/dlg/<rel>`), so the runtime scripting host (5.2+) reads
them from disk. Same bring-your-own-game posture as the maps/textures/sounds: output lives
under out/ (gitignored, regenerable).
...
Usage:
  python tools/UE_extract_scripts.py            # copy scripts + dialogue
  python tools/UE_extract_scripts.py --force    # re-copy even files already present
"""
import os
import sys
