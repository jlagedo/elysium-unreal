...
loose Python 2.1 level scripts 1:1 (see docs/roadmap.md decision log, 2026-07-22 5.5 entry).
The interpreter is open source (PSF), not game-sourced -- but it is ~37 MB, so like the rest
of tools/out it is regenerable, not committed. This script downloads the pinned release, verifies
...
Windows x64 only (the embed is Win64-only; elsewhere ELYSIUM_WITH_CPYTHON=0). Run:  python tools/fetch_cpython27.py
"""
...
# Pinned qnox/python-2.7 release (CPython 2.7.18, x86_64-pc-windows-msvc, MSC v.1944 / VS2022).
TAG = "v20260109"
ASSET = "cpython-2.7.18+20260109-x86_64-pc-windows-msvc-install_only.tar.gz"
URL = f"https://github.com/qnox/python-2.7/releases/download/{TAG}/{ASSET.replace('+', '%2B')}"
SHA256 = "0eab8de590076f74a17802d8b613ad74ec050b0081ccd0e2c7a47432c9b169b8"

REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DEST = os.path.join(REPO, "Source", "ElysiumUE", "ThirdParty", "CPython27")
...
def main():
    ...
    with open(os.path.join(DEST, "VERSION.txt"), "w") as f:
        f.write(f"cpython-2.7.18 {TAG} (qnox/python-2.7), x86_64-pc-windows-msvc\n"
                f"sha256 {SHA256}\n{URL}\n")
