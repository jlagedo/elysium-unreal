...
#
# The .ttf sources are placed by tools/fetch_ui_fonts.py (not by this script).
#
# **This generator is NOT in the content.bat umbrella, and cannot be.** Importing a `UFontFace`
# flushes Slate's font cache, and `FSlateApplication::Get()` asserts in a commandlet --
# `-run=pythonscript` never creates a Slate application (`-AllowCommandletRendering` does not
# help). Running it from `build_content.py` takes every `export_all.py` run down with it.
#
# It needs a Slate-enabled editor, and it is a rare regeneration -- only when a typeface is
# added or replaced -- so run it by hand from the editor's Python console:
#   py "E:/dev/elysium-unreal/tools/make_ui_fonts.py"
# and commit the resulting /Game/VtMB/UI/Fonts assets.
import os

import unreal

PKG = "/Game/VtMB/UI/Fonts"
SRC = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "Content", "Fonts")
...
def main():
    ...
    if missing:
        unreal.log_error(
            ...
            "run: python tools/fetch_ui_fonts.py" % (len(missing), ", ".join(missing)))
