...
per map, under tools/out/_profile/:
...
PROFILE_DIR = os.path.join(HERE, "out", "_profile")
...
def report(map_name: str) -> int:
    ...
    if not os.path.exists(summary_path):
        print(f"[profile] no summary at {summary_path} — run profile.bat {map_name} first.",
              file=sys.stderr)
        return 1
...
def main() -> int:
    ...
    ap.add_argument("--map", default="sp_tutorial_1", help="map name under tools/out")
    args = ap.parse_args()
