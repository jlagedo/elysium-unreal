...
plugin, or a source file that IntelliSense does not resolve.
    ...
    python tools/setup_vscode.py             # generate project files, then settings
    python tools/setup_vscode.py --no-ubt    # settings only, from the existing .rsp
"""
...
def find_ue_root() -> Path:
    """UE install path: $UE_ROOT, else the pin in build.bat."""
    env = os.environ.get("UE_ROOT")
    if env:
        return Path(env)
    build_bat = (REPO / "build.bat").read_text(encoding="utf-8", errors="replace")
    match = re.search(r'set\s+"UE_ROOT=([^"]+)"', build_bat, re.IGNORECASE)
    if not match:
        sys.exit("[vscode] could not read UE_ROOT from build.bat; set the UE_ROOT env var")
...
def main() -> None:
    ...
    settings["search.exclude"] = {f"**/{d}": True for d in EXCLUDE_DIRS} | {"tools/out": True}
