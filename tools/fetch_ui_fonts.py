...
Run once when a face is added or replaced; `content.bat` does **not** touch these
(they are static files, not editor-built `.uasset`s). Requires network.

    python tools/fetch_ui_fonts.py [--force]
"""
import argparse
import io
import os
...
def main():
    ...
    if any(not os.path.exists(os.path.join(dest, out)) or args.force
        ...
        try:
            ...
            sys.exit("fontTools is required to instance Inter: pip install -r tools/requirements.txt")
