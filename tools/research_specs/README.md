...
and remain under gitignored `tools/ghidra/`; a specification preserves enough
...
python tools/ghidra_context.py tools/research_specs/animation_pose.json `
  --binary "E:\path\to\Vampire\cl_dlls\client.dll"
python tools/ghidra_context.py tools/research_specs/animation_pose.json `
  --address 10091110 --kinds funcs,asm,xrefs
python tools/ghidra_context.py tools/research_specs/animation_pose.json --index-only
```
