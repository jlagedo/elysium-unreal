"""Copy the game's sign/popup definitions + decode their background art into out/signs.
...
Output is map-independent, so it lands in the global `out/signs/` folder (next to the
per-map `out/<map>/` dirs), read 1:1 by the runtime. No coordinate space is involved (2D
...
Produces (under tools/out/signs/):
...
Usage:
  python tools/UE_extract_signs.py            # copy definitions + decode backgrounds
  python tools/UE_extract_signs.py --force    # redo files already present
"""
import json
import os
import re
...
OUT = os.path.join(TOOLS, "out", "signs")
