...
Writes tools/out/testbox/ (obj + mtl + tex/checker.png + .lights + .spawn), which
the viewer loads like any exported map (ContentPaths anchors to tools/out). The OBJ
is authored directly in Godot space (Y-up, metres) — the viewer applies no further
transform to world OBJs.

  python tools/make_testmap.py
"""
import os
...
H = 6.0          # wall height
OUT = os.path.join(os.path.dirname(__file__), "out", "testbox")
