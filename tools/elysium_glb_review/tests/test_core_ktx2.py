...
class HeaderTests(unittest.TestCase):
    def test_level_index_begins_after_the_thirty_two_byte_index_block(self) -> None:
        ...
        self.assertEqual(ktx2.LEVEL_INDEX_OFFSET, 80)
    ...
    def test_parses_the_declared_header_fields(self) -> None:
        ...
        self.assertEqual(texture.vk_format, ktx2.VK_BC3_UNORM)
        self.assertEqual((texture.width, texture.height), (64, 32))
        self.assertEqual(texture.level_count, 4)
        self.assertEqual(len(texture.levels), 4)
        self.assertEqual(texture.format.fourcc, b"DXT5")

    def test_rejects_bytes_that_are_not_ktx2(self) -> None:
        with self.assertRaises(ktx2.Ktx2Error):
...
class LevelIndexTests(unittest.TestCase):
    def test_level_zero_is_the_largest_level(self) -> None:
        ...
        texture = ktx2.parse(data)
        self.assertEqual((texture.levels[0].width, texture.levels[0].height), (64, 64))
        self.assertEqual((texture.levels[4].width, texture.levels[4].height), (4, 4))
    ...
    def test_level_dimensions_floor_at_one(self) -> None:
        ...
        texture = ktx2.parse(data)
        self.assertEqual([(l.width, l.height) for l in texture.levels],
                         [(8, 1), (4, 1), (2, 1), (1, 1)])
    ...
    def test_level_count_zero_still_yields_one_entry(self) -> None:
        ...
        texture = ktx2.parse(data)
        self.assertEqual(texture.level_count, 0)
        self.assertEqual(len(texture.levels), 1)


class ImageSlicingTests(unittest.TestCase):
    def test_a_plain_two_dimensional_texture_has_one_image_per_level(self) -> None:
        data, images = support.build_ktx2(width=16, height=16, levels=3)
        texture = ktx2.parse(data)
        self.assertEqual(texture.images_per_level, 1)
        for level in range(3):
            with self.subTest(level=level):
                self.assertEqual(ktx2.image_bytes(data, texture, level), images[level])

    def test_cubemap_faces_slice_in_declared_order(self) -> None:
        data, images = support.build_ktx2(faces=6, width=8, height=8, levels=1)
        texture = ktx2.parse(data)
        self.assertTrue(texture.is_cubemap)
        self.assertEqual(texture.images_per_level, 6)
        for face in range(6):
            with self.subTest(face=face):
                self.assertEqual(ktx2.image_bytes(data, texture, 0, face=face), images[face])

    def test_array_layers_are_the_outer_loop(self) -> None:
        ...
        self.assertEqual(texture.images_per_level, 18)
        self.assertEqual(ktx2.image_bytes(data, texture, 0, layer=2, face=1), images[2 * 6 + 1])

    def test_layer_count_zero_means_one_element_not_none(self) -> None:
        data, _images = support.build_ktx2(layers=0)
        texture = ktx2.parse(data)
        self.assertEqual(texture.layers, 1)
        self.assertFalse(texture.is_array)

    def test_rejects_indices_outside_the_declared_ranges(self) -> None:
        data, _images = support.build_ktx2(faces=6, layers=2, levels=2)
        texture = ktx2.parse(data)
        for kwargs in ({"level": 9}, {"face": 6}, {"layer": 2}):
            with self.subTest(**kwargs):
                with self.assertRaises(ktx2.Ktx2Error):
                    ktx2.image_bytes(data, texture, **kwargs)

    def test_refuses_supercompressed_payloads_rather_than_returning_garbage(self) -> None:
...
class ExpectedSizeTests(unittest.TestCase):
    def test_block_sizes_match_the_declared_format(self) -> None:
        cases = {
            ktx2.VK_BC1_RGBA_UNORM: 8,
            ktx2.VK_BC2_UNORM: 16,
            ktx2.VK_BC3_UNORM: 16,
        }
        for vk_format, block_bytes in cases.items():
            with self.subTest(vk_format=vk_format):
                data, _images = support.build_ktx2(vk_format=vk_format, width=8, height=8)
                texture = ktx2.parse(data)
                # 8x8 is four 4x4 blocks.
                self.assertEqual(ktx2.expected_image_bytes(texture, 0), 4 * block_bytes)

    def test_uncompressed_rows_are_tightly_packed(self) -> None:
        # Three-byte texels get no padding to a four-byte boundary.
        data, _images = support.build_ktx2(
            vk_format=ktx2.VK_R8G8B8_UNORM, width=5, height=3, levels=1
        )
        texture = ktx2.parse(data)
        self.assertEqual(ktx2.expected_image_bytes(texture, 0), 5 * 3 * 3)

    def test_declared_and_actual_image_sizes_agree(self) -> None:
        data, _images = support.build_ktx2(width=32, height=16, levels=4, faces=6)
        texture = ktx2.parse(data)
        for level in range(4):
            with self.subTest(level=level):
                self.assertEqual(
                    len(ktx2.image_bytes(data, texture, level)),
                    ktx2.expected_image_bytes(texture, level),
                )

    def test_an_unknown_format_is_named_rather_than_guessed(self) -> None:
        data, _images = support.build_ktx2()
        broken = bytearray(data)
        struct.pack_into("<I", broken, 12, 999)
        texture = ktx2.parse(bytes(broken))
        self.assertIsNone(texture.format)
        with self.assertRaises(ktx2.Ktx2Error):
