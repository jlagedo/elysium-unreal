    ...
    def test_a_plain_two_dimensional_texture_has_one_image_per_level(self) -> None:
        ...
        for level in range(3):
            with self.subTest(level=level):
                assert ktx2.image_bytes(data, texture, level) == images[level]
    ...
    def test_cubemap_faces_slice_in_declared_order(self) -> None:
        ...
        for face in range(6):
            with self.subTest(face=face):
                assert ktx2.image_bytes(data, texture, 0, face=face) == images[face]
...
class ExpectedSizeTests(unittest.TestCase):
    ...
    def test_declared_and_actual_image_sizes_agree(self) -> None:
        ...
        for level in range(4):
            with self.subTest(level=level):
                assert len(ktx2.image_bytes(data, texture, level)) == ktx2.expected_image_bytes(texture, level)
