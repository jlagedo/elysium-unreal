"""What each NPC's clip vocabulary actually resolves to, off `out/npc/npc_manifest.json`.

The manifest's own consistency check and the answer to "which idle does this NPC play" --
the question the runtime's default-idle policy asks (roadmap 8.5). Reads only `out/`; touches
no game install.
...
CLI:
  python tools/probe_npc_clips.py              # per-NPC rollup + the manifest invariants
  python tools/probe_npc_clips.py --acts       # the activity histogram over every baked clip
  python tools/probe_npc_clips.py <stem> ...   # one NPC's resolved idle set, in detail
"""
import json
import os
import re
...
def load():
    ...
    if m.get("manifest_version") != 2:
        sys.exit(f"{MANIFEST}: expected manifest_version 2, got {m.get('manifest_version')!r} "
                 f"-- re-run `python tools/npc_export.py`")
    return m
