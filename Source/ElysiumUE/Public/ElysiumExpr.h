...
// (field-6 call strings, logic_pythoncheck gates, and dlgexpr). It is the one
...
//
// The engine `vampire`-module surface: the 11 native module globals (`FindPlayer`,
// `FindEntityByName`, ...) resolve as bare names, and the 24 Character methods dispatch off a
// character object (`FindPlayer()` -> the PC; an NPC entity handle also accepts them). Unbacked
// methods log a stub and return a sensible default; the two that exist route for real
// (`SetQuest`/`GetQuestState` -> the quest map). `self`/`activator` resolve to the
// evaluation's I/O provenance when set. Level-script functions and constants (cCelerity,
// DialogPostProcess, the level's `On*` callbacks, ...) arrive with the level-script host;
// unbound, those names are unresolved -> NameError, so `G.x |= cCelerity` no-ops correctly.
//
...
// this is the confirmed error-to-false behaviour the pythoncheck/dialogue paths rely on.
namespace ElysiumExpr
{
	// The evaluation environment — the collaborators + the out-of-band diagnostic. `Ctx` carries the
	// I/O provenance the evaluation runs against (`!self`/`!activator`) and the entity world to
