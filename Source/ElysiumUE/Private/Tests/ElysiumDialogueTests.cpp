...
// The one corpus-tier case in this file: it reads the exported `.dlg` tree and the deployed
// rulebook, so it abstains rather than fails when neither has been produced yet.
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FElysiumDlgCorpusDependenciesTest,
	"Elysium.Content.DlgCorpusDependencies", GElysiumTestFlags)
bool FElysiumDlgCorpusDependenciesTest::RunTest(const FString&)
{
	const FString Dir = FElysiumContentPaths::DlgDir();
	TArray<FString> Files;
	if (!Dir.IsEmpty())
	{
		IFileManager::Get().FindFilesRecursive(Files, *Dir, TEXT("*.dlg"), true, false);
	}
	if (Files.IsEmpty())
	{
		AddInfo(FString::Printf(
			TEXT("ELYSIUM_TEST_ABSTAIN: no exported dialogue under %s (run the pipeline to enable)"),
			*Dir));
		return true;
	}

	// The resolver reads the real tables. `MakeTraitResolver(nullptr)` falls back to the headless
	// binding, which is the same seam `CalcFeat` uses when no GameInstance is up.
	FElysiumStatTable Stats;
	FElysiumFeatTable Feats;
	FString StatsError;
	FString FeatsError;
	const bool bStats = Stats.Load(StatsError);
	const bool bFeats = Feats.Load(FeatsError);
	if (!bStats || !bFeats)
	{
		AddInfo(FString::Printf(
			TEXT("ELYSIUM_TEST_ABSTAIN: the rulebook did not load (%s / %s)"),
			bStats ? TEXT("stats ok") : *StatsError, bFeats ? TEXT("feats ok") : *FeatsError));
		return true;
	}
	...
	ON_SCOPE_EXIT { ElysiumSheetRules::BindTables(ElysiumSheetRules::FBoundTables()); };

	TSharedRef<const IElysiumDlgTraitResolver> Resolver =
		ElysiumDlgSheet::MakeTraitResolver(nullptr);
	...
	TArray<FString> UnknownClass;
	TMap<FString, int32> ByTrait;
	for (const FString& File : Files)
	{
		FElysiumDlgFile Parsed;
		FString Error;
		if (!FElysiumDlgFile::LoadFile(File, Parsed, &Error))
		{
			AddError(FString::Printf(TEXT("%s: %s"), *File, *Error));
			continue;
		}
		for (const FElysiumDlgLine& Line : Parsed.Lines)
		{
			// col-4 is a GATE only on a PC row; on an NPC row it is an action and is never parsed.
			if (!Line.IsPcChoice() || Line.Condition.IsEmpty())
			{
				continue;
			}
			++Conditions;
			const FElysiumDlgDependency Dep =
				FElysiumDlgDependency::Parse(Line.Condition, &Resolver.Get());
			if (Dep.bUnresolvedSkillFront && Unresolved.Num() < 10)
			{
				Unresolved.Add(FString::Printf(TEXT("%s line %d: '%s'"),
					*FPaths::GetCleanFilename(File), Line.Id, *Line.Condition));
			}
			if (!Dep.bHasSkill)
			{
				continue;
			...
			++SkillFronts;
			Inverted += Dep.bInverted ? 1 : 0;
			Disciplines += Dep.Class == EElysiumDlgTraitClass::Discipline ? 1 : 0;
			ByTrait.FindOrAdd(Dep.Trait.ToLower())++;
			if (Dep.Class == EElysiumDlgTraitClass::Unknown && UnknownClass.Num() < 10)
			{
				UnknownClass.Add(FString::Printf(TEXT("%s line %d: '%s'"),
					*FPaths::GetCleanFilename(File), Line.Id, *Line.Condition));
			}
		}
	}
	...
	AddInfo(FString::Printf(
		TEXT("%d .dlg files, %d gated PC rows, %d skill fronts (%d inverted, %d disciplines) over %d trait names"),
		Files.Num(), Conditions, SkillFronts, Inverted, Disciplines, ByTrait.Num()));
	TestEqual(TEXT("every skill-shaped front resolves to a trait"),
		FString::Join(Unresolved, TEXT("; ")), FString());
	TestEqual(TEXT("no dependency carries an Unknown class"),
		FString::Join(UnknownClass, TEXT("; ")), FString());
	// The rewrite these replace left every one of these unrecognised, which is the whole of D1.
	TestTrue(TEXT("the corpus really does author negative thresholds"), Inverted > 400);
	TestTrue(TEXT("...and discipline checks with a blood price"), Disciplines > 100);

	// D4's corpus pin: `prince1.dlg` is the only file with a Ventrue column, and its 8 rows survive
	// the parse (a row is stored only when its id and male text pass `read_line_data`).
	int32 VentrueRows = 0;
	for (const FString& File : Files)
	{
		if (!FPaths::GetCleanFilename(File).Equals(TEXT("prince1.dlg"), ESearchCase::IgnoreCase))
		{
			continue;
		}
		FElysiumDlgFile Parsed;
		...
		if (FElysiumDlgFile::LoadFile(File, Parsed, &Error))
		{
			for (const FElysiumDlgLine& Line : Parsed.Lines)
			{
				VentrueRows += Line.TextClan[ElysiumDlgClan::Ventrue].IsEmpty() ? 0 : 1;
			}
		}
	}
	TestEqual(TEXT("prince1.dlg keeps its 8 Ventrue rows"), VentrueRows, 8);
