...
static const FVector2D Surface1024x768(1024.0, 768.0);
}
...
bool FElysiumTerminalCellMetricsTest::RunTest(const FString&)
{
	...
	// The retail grid on the project's 1024x768 target. `1024 / 36 = 28.44` floors to 28, so the
	// text block is 1008 wide and the 16 leftover pixels split into an 8 px margin either side;
	// `768 / 24 = 32` divides exactly, so there is no vertical margin at all.
	const FCellMetrics Retail = MetricsFor(Surface1024x768, 36, 24);
	TestTrue(TEXT("the 36x24 grid on a 1024x768 target is drawable"), Retail.bValid);
	TestEqual(TEXT("cell width floors to 28"), Retail.CellWidth, 28);
	TestEqual(TEXT("cell height is exactly 32"), Retail.CellHeight, 32);
	TestEqual(TEXT("the text block is 1008 wide"), Retail.BlockWidth(), 1008);
	TestEqual(TEXT("and 768 tall"), Retail.BlockHeight(), 768);
	TestEqual(TEXT("centred, so the x origin is 8"), Retail.OriginX, 8);
	TestEqual(TEXT("and the y origin is 0"), Retail.OriginY, 0);
	TestTrue(TEXT("cell (0,0) sits at the origin"),
		Retail.TopLeftOf(0, 0).Equals(FVector2f(8.0f, 0.0f)));
	TestTrue(TEXT("and the last cell at the far corner of the block"),
		Retail.TopLeftOf(35, 23).Equals(FVector2f(8.0f + 35 * 28, 23.0f * 32)));

	// The metrics are derived, not tabulated: a `textcolumns 4` / `textrows 2` terminal — inside
	// the `[4,36] x [2,24]` clamp `CBaseTerminal::Spawn` applies — gets its own cell size.
	const FCellMetrics Small = MetricsFor(Surface1024x768, 4, 2);
	TestEqual(TEXT("a 4x2 grid takes 256-wide cells"), Small.CellWidth, 256);
	TestEqual(TEXT("and 384-tall ones"), Small.CellHeight, 384);
	TestEqual(TEXT("with no margin left over"), Small.OriginX + Small.OriginY, 0);

	// Degenerate inputs draw nothing rather than dividing by zero.
	TestFalse(TEXT("a zero column count is refused"),
		MetricsFor(Surface1024x768, 0, 24).bValid);
	TestFalse(TEXT("a zero row count is refused"), MetricsFor(Surface1024x768, 36, 0).bValid);
	TestFalse(TEXT("a zero-sized surface is refused"),
		MetricsFor(FVector2D::ZeroVector, 36, 24).bValid);
	TestFalse(TEXT("more cells than pixels is refused"),
...
bool FElysiumTerminalCellPaintTest::RunTest(const FString&)
{
	...
	{
		const FDrawPlan Plan = BuildDrawPlan(View, FString(), Surface1024x768, 0.0f, false);
		...
		TestTrue(TEXT("...and follows it when it changes"),
			BuildDrawPlan(View, FString(), Surface1024x768, 0.0f, false)
	...
	// A run that reaches the right edge closes at the edge rather than running off it.
	{
		...
		const FDrawPlan Plan = BuildDrawPlan(EdgeView, FString(), Surface1024x768, 0.0f, false);
	...
	// --- an unpublished grid draws nothing rather than one screen-wide inverse block --------------
	{
		...
		const FDrawPlan Plan = BuildDrawPlan(Empty, FString(), Surface1024x768, 0.0f, false);
	...
	{
		const FDrawPlan Plan = BuildDrawPlan(Session, TEXT("cho"), Surface1024x768, 0.0f, false);
	...
	}
	{
		...
		TestFalse(TEXT("the cursor is dark on the second half of the blink"),
			BuildDrawPlan(Session, TEXT("cho"), Surface1024x768, 0.6f, false).bCursorVisible);
	}
	{
		...
		TestFalse(TEXT("acknowledge mode draws no cursor"),
			BuildDrawPlan(Acknowledge, FString(), Surface1024x768, 0.0f, false).bCursorVisible);
		...
		TestFalse(TEXT("a closed client line editor draws no cursor"),
			BuildDrawPlan(Closed, FString(), Surface1024x768, 0.0f, false).bCursorVisible);
		FElysiumTerminalView Idle = Session;
		Idle.SessionSerial = 0;
		TestFalse(TEXT("an idle monitor draws no cursor"),
			BuildDrawPlan(Idle, FString(), Surface1024x768, 0.0f, false).bCursorVisible);
	}

	// --- the calibration pattern -----------------------------------------------------------------
	{
		const FDrawPlan Plan = BuildDrawPlan(Session, TEXT("cho"), Surface1024x768, 0.0f, true);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FElysiumTerminalFontRoleTest,
	"Elysium.Substrate.Terminal.FontRole", GCellFlags)
bool FElysiumTerminalFontRoleTest::RunTest(const FString&)
{
	// The generated faces are editor products (`pipeline/unreal/make_ui_fonts.py`), so a tree that
	// has not run the generator abstains rather than failing: the layout still works there, drawn
	// in Slate's default face, which is the library's own documented fallback.
	const FString RegularPath = FElysiumContentPaths::UiFontFace(TEXT("FF_TerminusTTF_Regular"));
	const FString BoldPath = FElysiumContentPaths::UiFontFace(TEXT("FF_TerminusTTF_Bold"));
	if (!LoadObject<UFontFace>(nullptr, *RegularPath))
	{
		AddInfo(FString::Printf(
			TEXT("ELYSIUM_TEST_ABSTAIN: %s is absent — run pipeline/unreal/make_ui_fonts.py"),
			*RegularPath));
		return true;
	}

	const FSlateFontInfo Mono = ElysiumUIFonts().Font(EElysiumFontRole::Mono,
		EElysiumFontWeight::Regular, 30.0f, 1.0f);
	const UFont* Composed = Cast<UFont>(Mono.FontObject);
	if (!TestNotNull(TEXT("the Mono role composes a runtime UFont"), Composed))
	{
		return false;
	}
	TestEqual(TEXT("drawn at the cell size it was asked for"), Mono.Size, 30.0f);
	TestEqual(TEXT("under the Regular typeface entry"), Mono.TypefaceFontName, FName(TEXT("Regular")));

	const TArray<FTypefaceEntry>& Entries = Composed->GetCompositeFont()->DefaultTypeface.Fonts;
	if (!TestTrue(TEXT("with at least the regular face registered"), Entries.Num() >= 1))
	{
		return false;
	}
	TestEqual(TEXT("the first entry is Regular"), Entries[0].Name, FName(TEXT("Regular")));
	TestEqual(TEXT("and it is FF_TerminusTTF_Regular, not one of the three text families"),
		GetNameSafe(Entries[0].Font.GetFontFaceAsset()), FString(TEXT("FF_TerminusTTF_Regular")));
	if (LoadObject<UFontFace>(nullptr, *BoldPath) && Entries.Num() >= 2)
	{
		// Terminus ships no italic; Bold takes the library's one heavier slot.
		TestEqual(TEXT("the heavier slot carries FF_TerminusTTF_Bold"),
			GetNameSafe(Entries[1].Font.GetFontFaceAsset()), FString(TEXT("FF_TerminusTTF_Bold")));
	}

	// The role is not the body role: a terminal that fell back onto Spectral would still lay out.
	const FSlateFontInfo Body = ElysiumUIFonts().Font(EElysiumFontRole::Body,
		EElysiumFontWeight::Regular, 30.0f, 1.0f);
	TestTrue(TEXT("Mono resolves to its own UFont, not the body role's"),
		Body.FontObject != Mono.FontObject);
	return true;
}
