...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FElysiumSurfaceSoundTableTest,
	"Elysium.Content.SurfaceSoundTable", GElysiumTestFlags)
bool FElysiumSurfaceSoundTableTest::RunTest(const FString&)
{
	// The 63 assets are git-tracked under the ElysiumBaked plugin, so this reads the checkout
	// rather than the export root — a fresh clone has them and needs no exporter run.
	...
	// The query is answered through the REAL `IElysiumEmbodiment` implementation, on the class
	// default object: `AElysiumMapActor::ResolveSurfaceSounds` reads no actor state at all (the load
	// cache behind it is session-scoped, not per-map), so a spawned actor and a world would prove
	// nothing extra. Same shape the entity-lifecycle suite uses to read the CDO.
	...
	// --- Every baked entry resolves ------------------------------------------------------------
	int32 Resolved = 0;
	int32 WithPools = 0;
	TArray<FString> Unresolved;
	for (const FString& Leaf : Files)
	{
		FString Name = FPaths::GetBaseFilename(Leaf);
		Name.RemoveFromStart(TEXT("PM_"));
		FElysiumSurfaceSounds Row;
		if (Embodiment.ResolveSurfaceSounds(FName(*Name), Row))
		{
			++Resolved;
			WithPools += Row.HasSteps() ? 1 : 0;
		}
		else
		{
			Unresolved.Add(Name);
	...
	TestEqual(TEXT("every baked surfaceprop resolves through the seam"), Resolved, Files.Num());
	if (!Unresolved.IsEmpty())
	{
		AddError(FString::Printf(TEXT("surfaces that did not resolve: %s"),
			*FString::Join(Unresolved, TEXT(", "))));
	}
	// 46 of the 63 carry a step pool once the `base` chain is flattened; the exact number is
	// asserted as a floor plus the declared set below, so adding a surface does not break this.
	TestTrue(TEXT("the flattened table carries at least the 17 declared step pools"),
		WithPools >= static_cast<int32>(UE_ARRAY_COUNT(GDeclaredStepPools)));

	// --- The 17 declared pools keep their sizes ------------------------------------------------
	for (const FDeclaredPool& Expected : GDeclaredStepPools)
	{
		FElysiumSurfaceSounds Row;
		if (!TestTrue(FString::Printf(TEXT("%s resolves"), Expected.Surface),
			Embodiment.ResolveSurfaceSounds(FName(Expected.Surface), Row)))
		{
			continue;
		}
		TestEqual(FString::Printf(TEXT("%s stepleft pool size"), Expected.Surface),
			Row.StepLeft.Num(), Expected.Left);
		TestEqual(FString::Printf(TEXT("%s stepright pool size"), Expected.Surface),
			Row.StepRight.Num(), Expected.Right);
	}

	// --- The rows are engine-relative paths, the gamematerial letter and the scripts -----------
	{
		FElysiumSurfaceSounds Concrete;
		if (TestTrue(TEXT("concrete resolves"),
			Embodiment.ResolveSurfaceSounds(FName(TEXT("concrete")), Concrete)))
		{
			// The `vtmb:sound:` id is stripped at the seam: what crosses is what `PlayVoice` takes.
			TestEqual(TEXT("concrete's first left step is the engine-relative wav"),
				Concrete.StepLeft.IsEmpty() ? FString() : Concrete.StepLeft[0],
				FString(TEXT("surfaces/concrete/stepleft1.wav")));
			TestFalse(TEXT("and carries no asset-id prefix"),
				Concrete.StepLeft.IsEmpty()
					|| Concrete.StepLeft[0].StartsWith(TEXT("vtmb:")));
			TestEqual(TEXT("concrete's gamematerial letter"),
				Concrete.GameMaterial, FString(TEXT("C")));
		}

		// Flattening reaches the seam: `brick` declares no step key of its own and bases on
		// `concrete`, so it must answer with concrete's four alternates.
		FElysiumSurfaceSounds Brick;
		if (TestTrue(TEXT("brick resolves"),
			Embodiment.ResolveSurfaceSounds(FName(TEXT("brick")), Brick)))
		{
			TestEqual(TEXT("brick inherits concrete's four left alternates"),
				Brick.StepLeft.Num(), 4);
			TestEqual(TEXT("...and its four right ones"), Brick.StepRight.Num(), 4);
		}

		// The name is folded, so either spelling of an entry finds the same asset.
		FElysiumSurfaceSounds Upper;
		TestTrue(TEXT("a capitalised surface name resolves to the same entry"),
			Embodiment.ResolveSurfaceSounds(FName(TEXT("Concrete")), Upper));
	}

	// --- The two answers a producer has to be able to tell apart -------------------------------
	FElysiumSurfaceSounds Nothing;
	TestFalse(TEXT("NAME_None is retail's null surfacedata_t"),
		Embodiment.ResolveSurfaceSounds(NAME_None, Nothing));
	TestFalse(TEXT("and an unbaked name answers the same way"),
		Embodiment.ResolveSurfaceSounds(FName(TEXT("no_such_surface")), Nothing));
