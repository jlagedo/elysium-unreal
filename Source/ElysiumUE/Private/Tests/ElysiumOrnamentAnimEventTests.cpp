...
// --- The other half: a real baked clip carrying the record ----------------------------------------
//
// The substrate cases above prove what the handler does with a 4102. They cannot prove that any clip
// this runtime plays ever fires one — the whole chain from `mstudioevent_t` through the bake to
// `FElysiumBlendTable::Events` sits between them, and every link of it is silent when it breaks.
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FElysiumOrnamentAnimEventRecordsTest,
	"Elysium.Content.OrnamentAnimEventRecords", GElysiumTestFlags)
bool FElysiumOrnamentAnimEventRecordsTest::RunTest(const FString&)
{
	...
	// The shared male `misc` bank — `models/character/shared/male/misc.mdl`, which is where the
	// interesting-place idles (`cigarette`, `cellphone`, `can_drink`) live.
	const FString Bank = TEXT("character_shared_male_misc");
	FString Error;
	if (ElysiumCharacterAssets::Body(Bank, Error) == nullptr)
	{
		AddInfo(FString::Printf(
			TEXT("ELYSIUM_TEST_ABSTAIN: the shared male misc bank is not baked (%s): %s"),
			*Bank, *Error));
		return true;
	}
	...
	// Every ornament record the bank carries, so a failure names what IS there rather than only what
	// is not. The ids are the whole `m_hAnimFollowModel` family.
	TArray<FString> Rows;
	int32 Attaches = 0;
	for (const auto& Pair : Table.Events)
	{
		for (const FElysiumAnimEvent& Record : Pair.Value)
		{
			if (Record.Event != ElysiumAnimEvents::AttachFollowModel
				&& Record.Event != ElysiumAnimEvents::DetachFollowModel
				&& Record.Event != ElysiumAnimEvents::AttachFollowModelGendered)
			{
				continue;
			}
			++Attaches;
			Rows.Add(FString::Printf(TEXT("%s: %d @ %.4f '%s'"),
				*Pair.Key, Record.Event, Record.Cycle, *Record.Options));
		}
	}
	Rows.Sort();
	for (const FString& Row : Rows)
	{
		AddInfo(FString::Printf(TEXT("ornament record — %s"), *Row));
	}
	if (!TestTrue(FString::Printf(TEXT("%s carries ornament records at all"), *Bank), Attaches > 0))
	{
		return false;
	}

	// `cigarette_Into` is the interesting-place entry clip: it is where the cigarette first appears,
	// so it is the record whose absence would be silent — the idle re-fires 4102 every loop, and a
	// missing entry record would only show as a body that lights up one loop late.
	const TArray<FElysiumAnimEvent>* Into = Table.FindEvents(TEXT("cigarette_Into"));
	if (Into == nullptr)
	{
		AddInfo(TEXT("ELYSIUM_TEST_ABSTAIN: the misc bank declares no 'cigarette_Into' timeline"));
		return true;
	}
	const FElysiumAnimEvent* Attach = Into->FindByPredicate([](const FElysiumAnimEvent& Record)
		{ return Record.Event == ElysiumAnimEvents::AttachFollowModelGendered; });
	if (!TestNotNull(TEXT("cigarette_Into carries a 4102 record"), Attach))
	{
		return false;
	}
	TestFalse(TEXT("...whose options name the model path the handler formats from"),
		Attach->Options.IsEmpty());

	// The formatted key, end to end: what the runtime would look up in `DA_OrnamentModels` for this
	// authored record on a male body. It is asserted as a VALUE rather than only as non-empty,
	// because the doubled-extension case proves the format is applied verbatim.
	const FString Formatted = ElysiumAnimEvents::FormatFollowModelPath(
		Attach->Event, Attach->Options, /*bMale*/ true);
	AddInfo(FString::Printf(TEXT("cigarette_Into 4102 '%s' -> catalogue key '%s'"),
		*Attach->Options, *Formatted));
	TestTrue(TEXT("...and it formats a lowercased .mdl path"),
		Formatted.EndsWith(TEXT(".mdl")) && Formatted == Formatted.ToLower());
	return true;
}
}
