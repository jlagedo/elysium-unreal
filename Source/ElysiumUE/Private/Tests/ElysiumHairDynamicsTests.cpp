...
{
	TestNull(TEXT("hair dynamics has no runtime feature CVar"),
		IConsoleManager::Get().FindConsoleVariable(TEXT("elysium.HairDynamics")));

	const UElysiumHairDynamicsConfig* Config = UElysiumHairDynamicsConfig::Load();
	if (!TestNotNull(TEXT("the tracked authored hair table loads"), Config))
	{
		return true;
	}
	...
	for (const TPair<FName, FElysiumHairDynamicsStem>& Entry : Config->Stems)
	{
		...
		for (const FElysiumHairDynamicsChain& Chain : Entry.Value.Chains)
		{
			TestTrue(*FString::Printf(TEXT("%s: authored bound bone %s exists"),
				*Entry.Key.ToString(), *Chain.BoundBone.ToString()),
				RefSkeleton.FindBoneIndex(Chain.BoundBone) != INDEX_NONE);
			TestTrue(*FString::Printf(TEXT("%s: authored chain end %s exists"),
				*Entry.Key.ToString(), *Chain.ChainEnd.ToString()),
				RefSkeleton.FindBoneIndex(Chain.ChainEnd) != INDEX_NONE);
		}
	}
	return true;
}
