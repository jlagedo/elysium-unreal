...
namespace
{
	...
	// The animation producer, per frame — body sample.
	// The body sample's two candidate movement yaws, both facing-relative. **Both are recorded on
	// purpose**: the speed authority recovers the sign of `move_yaw` by comparing the retail
	// selector's own input against them, and that comparison runs against recordings rather
	...
	// The animation producer, per frame — selection.
	...
	// What the MOTOR was commanding while that cell played. It is the third number the
	...
	// formality here: the damper's position is a function of how fast the body moved, so the speed
	// authority really can move all of them. Their comparison therefore lives in the sited runs —
	// whose baselines are full traces — and in the cross-rate `--hz` run, which is what actually
	// asserts that the damper settles the same way at 60 and 120 Hz.
	...
	// enough course. That is what makes them independent of the speed authority when no per-frame
	// trace can.
	...
	// The leniency measurement, per run.
	// Written only by the courses that place a jump against a body event, because a course that
	// merely *holds* jump re-fires on landing at a gait-dependent moment and its count would
	// move with the speed authority. On a leniency course the count is 0 or 1 at any gait, which is what makes it
	...
	// The animation producer, per run.
	...
	// moves with the speed authority.
	...
	// The no-slide verdict, carried by the recording itself. It saturates at zero the way
