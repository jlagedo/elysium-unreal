...
`Private/` is subfoldered **by layer**, and the folder a file sits in is the statement of which
layer it belongs to. A private header is included by its layer path — `#include
"Visual/ElysiumLightRig.h"` — so an include line names the layer it reaches into and a
...
| `Substrate/` | Track B — the plain-C++ entity object model, every entity class, I/O, movers, expressions |
| `Scripting/` | the CPython VM, the script hosts and natives, the `.dlg` conversation machine |
| `Visual/` | everything that produces or tunes what is **rendered** — materials, textures, the light rig, the sky/fog, decals, prop skins, NPC meshes and animation, ropes, the map's look, entity bodies |
...
- `Config/DefaultInput.ini` — engine-side input settings only: `ConsoleKeys` = `Tilde` + `F7`, raw
  ...
  mappings — every key is installed by `UElysiumInputRouter` from `ElysiumBinds::Defaults()`.
  EnhancedInput is configured as the player-input class, with its own mapping contexts.
- Boot is decided once, at game-instance init, by `UElysiumGameFlowSubsystem::BootFromCommandLine`
  ...
  **mock character** (`BeginNewGame(Tremere, male)` — interim stand-in for chargen) so the
...
which calls `SpawnPendingMap`) → `AElysiumMapActor`, which owns the map for one epoch — the engine
tears the world down on travel and GC frees it, no manual flush, no force-GC. `/Game/Elysium` is
...
`UElysiumMapCollision` builds the walkable surface from `.hulls` (one convex `FKConvexElem` per
solid world brush, PLAYERCLIP included) + `.dispcol` (displacement trimesh) onto collision-only
PMCs, created at load so a map with neither sidecar builds neither; `elysium.BrushCollision 0`
skips the pass and leaves the map with no walkable surface (a debugging flythrough).
...
| `LoadGame` / `SaveGame` | the seam every caller (menu, `trigger_autosave`, quicksave, MCP) goes through; both delegate to `UElysiumSaveSubsystem`. `SaveGame` with no slot resolves by kind; `LoadGame` with no slot takes the newest on disk |
...
`elysium.pause` still survives a travel.
...
actor's build pass runs after `PostLoadMapWithWorld`.

**Esc is one verb with two key sources.** Both fire **`cancelselect`**, whose single
...
death path: the player entity's health running out reaches
`UElysiumGameStateSubsystem::NotifyPlayerKilled` → `TriggerGameOver(Killed)`. The masquerade meter is
the second loss condition and is not yet implemented. `elysium.gameover` is the named command that reaches
...
| `FElysiumCameraShotStack` | the scripted channel: **handle-based** push/pop (shots end out of order, like the input scopes), a **timed** ramp off each shot's own duration, and `Update` to refresh a live shot without restarting its blend — which is what a `Follow` attach type is. What is pushed is **values** (origin, look-at, roll, FOV, `MoveSpeed`/`MaxTurnRate`), so the camera never learns what an entity is |
...
There is **no player mesh yet**: the fade band is solved and exposed as `ModelAlpha()` and
nothing reads it. Weapon-class arbitration is not yet implemented (the cvars are declared so a config round-trips);
...
| `ElysiumSaveTypes.h` | the four blocks as plain structs — `FElysiumSessionBlock` (clock, `G`, quests, the RNG snapshot), `FElysiumPlayerRecord` (reused whole), `TMap<FString, FElysiumMapSnapshot>`, `FElysiumWorldBlock` — plus `FElysiumEntityState`, `FElysiumSavedFade`, `FElysiumSaveHeaderData` and `FElysiumSaveVersion` (schema id, `MinSupported` floor, a registered engine custom version) |
...
flag, and resolves a mid-swing save to its destination pose.
...
focus and mapping contexts. LocalPlayer-scoped because the stack has to survive travel and
...
| `Cinematic` / `Chargen` | nothing yet — P12 | priorities reserved |
...
pushed *because* the flow paused. Verb: `elysium.inputscopes` dumps
...
the same null-service discipline already established.
...
`+speed` in the frame's `FElysiumUserCmd`, not a key poll, so the pawn does not tick.
...
the map epoch's lifetime and is saved with it — and the world *announces* to the presenter in addition to
...
The substrate reaches them **through the player entity**, not directly: `point_teleport`
...
`GetPlayerViewPoint` is the eye. **`PushCameraShot`/`PopCameraShot`** are here for the same
...
its debug overlays, which is exactly when they are read. `AElysiumHUD` itself does not tick.
...
classnames, over the character chain, and `FElysiumNpcMaker` for
`npc_maker`/`npc_maker_fleshpile`), `ElysiumPlayerClasses.cpp` (`CBaseAnimating`,
`CBaseCombatCharacter` and the `player` leaf),
`ElysiumScriptedSequence.cpp` (`FElysiumScriptedSequence` for
`scripted_sequence`/`aiscripted_sequence`), and
`ElysiumPropClasses.cpp` (`FElysiumProp` for `prop_dynamic`/`prop_dynamic_ornament`;
...
shared glb→`USkeletalMesh` loader (reused by the `UElysiumNpcSubsystem` test harness)
...
**Animation.** A VtMB NPC's own `.mdl` carries only its own clips — mostly dialogue — and
...
an NPC's stance follows the conversation. The emotional-state half is still unimplemented, and the natives
...
**Dialogue (B4).** `ElysiumDlg.{h,cpp}` is the engine-neutral core: the 13-field `.dlg` parser
...
UI-only input scope; `elysium.dlg`/`.choose`/`.advance` are its scriptable echo (`ElysiumDlgConsole.cpp`).
A later pass replaces the box on the UI stack.

Dynamic props (no physics) stand a static-mesh body the same way: `FElysiumProp`
(`prop_dynamic`/`prop_dynamic_ornament`) calls `IElysiumEmbodiment::BuildPropVisual` — resolve the baked
`SM_<stem>` through `ResolvePropMesh` (the `model_mesh` annotation names the stem; cached per stem,
...
class desc that registers **only** the `skin` field — a body and its skin, no invented I/O.
...
Physics props stand a **simulating** Chaos body: `FElysiumPhysProp` (`prop_physics`) calls
...
surface is `Wake` (real), `Break` (hide + `OnBreak`), and `Skin`/`SetSkin`/
...
`OnStartTouch`/`OnEndTouch`, and the activator it carries is the player entity's handle, so
...
`IsPlayerSeated` gate is retired).

## Player-facing UI

**CommonUI + CommonInput, with widget trees built in C++ Slate** — no Widget Blueprint assets and
no editor content loop. Design + the constraints that shaped it:
`docs/ui-architecture.md`; what VtMB's own UI is: `docs/vtmb-ui.md`.

| Type | Role |
|---|---|
| `UElysiumUISubsystem` | GI-scoped owner of the screens: create/show/hide, plus the `Menu` input scope it pushes while one is up (the scope names the menu widget as its focus target, so the screen can see Escape). GI-scoped because the menu outlives any one world. *When* a screen is up is not its call — `UElysiumGameFlowSubsystem` drives it from the app state. `elysium.menu [pause\|gameover]` / `elysium.menu.close` |
| `UElysiumMainMenu` | the main / pause / game-over menu (`UCommonActivatableWidget`), one `EElysiumMenuMode` per item set; a mode change rebuilds the screen. **Two layouts**, A/B'd live by `elysium.MenuLayout`, both in a 1024×768 virtual canvas under one `SDPIScaler` at `ScreenH/768`: the **rail** (1, default — right-aligned column against a gold hairline, a local veil instead of a global scrim, bone-at-rest/blood-when-armed, one tick eased onto the armed row by `NativeTick`, and a `mm_<clan>` seal off the menu sprite sheet) and the **classic** `CVMainMenu::PerformLayout` reproduction (0 — widest label + `20×4`, `pitch = height + 2`, centred, blood at rest, `elysium.MenuScrim`). Rail horizontals are measured from the **right edge**, never as a fraction of 1024, because the virtual canvas is `ScreenW·768/ScreenH` wide. A dead row stays *enabled* so it can arm and show its caption; the click is gated instead. Every live item calls the flow subsystem. **A `UCommonActivatableWidget` added straight to the viewport stays collapsed until `ActivateWidget()`** (`bAutoActivate` only fires inside a `UCommonActivatableWidgetContainer`) |
...
| `ElysiumUITexture.{h,cpp}` | PNG → transient BGRA texture, shared by the use-icon atlas, the sign backgrounds, the title lockup and the menu seal — plus `MakeAlphaRamp`, a white texture carrying an alpha ramp whose colour comes from the brush tint (the menu rail's veil, hairline and tick), so a gradient is authored beside the layout constants it was tuned against rather than as an asset that can drift from them |
...
- `ElysiumPythonEntity.{h,cpp}` is the `vampire` module's object surface: the **`Entity`** type
  ...
  globals, and the console objects **`ccmd`/`cvar`**. **There is no `Player` type** — it was
  retired; `FindPlayer()` returns an `Entity` over the player entity, so `pc.clan` is a field and
  ...
  `GetDynamicField` hook (the `vdata` half of the sheet, `pc.base_*`, not yet given real names),
  ...
  a separate table that survives a re-seed and is shadowed by any cfg carrying the name, so the
...
- `ElysiumScriptNatives.{h,cpp}` is the engine `vampire` surface both hosts share: the binding
  ...
  read the player sheet clan/gender), `CurrentMoney` reads the receiver's own `money` field,
  ...
  exactly one (`elysium.script.oneofset` pins the roll). The rest of the
  character surface (inventory, blood, humanity/XP, `CalcFeat`) is a logged stub.
